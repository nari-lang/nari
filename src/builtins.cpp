...
bool utf8_to_utf16(const std::string& input, std::u16string& output) {
    ...
    while (i < input.size()) {
        ...
        if ((c & 0x80u) == 0) {
            ...
            if (codepoint < 0x800u) return false;
...
// this is where builtin functions live (functions that call back into native code)

Value ScriptRuntime::builtin_print(const std::vector<Value> &argvals, const nari::CallExpr*) {
...
Value ScriptRuntime::builtin_setTimeout(const std::vector<Value> &argvals, const nari::CallExpr*) {
    if (argvals.size() >= 2) {
        ...
        if (io_pool) io_pool->submit(io_op);
    }
    return Value::none();
}

Value ScriptRuntime::builtin_setInterval(const std::vector<Value> &argvals, const nari::CallExpr*) {
    if (argvals.size() >= 2) {
        Value callback_val = argvals[0];
        int64_t delay_ms = 0;
...
Value ScriptRuntime::builtin_clearInterval(const std::vector<Value> &argvals, const nari::CallExpr*) {
...
// Math builtins
Value ScriptRuntime::builtin_math_sqrt(const std::vector<Value> &argvals, const nari::CallExpr*) {
    if (!argvals.empty()) {
        double num = 0.0;
        if (argvals[0].is_int()) {
...
Value ScriptRuntime::builtin_math_rand(const std::vector<Value> &argvals, const nari::CallExpr*) {
...
Value ScriptRuntime::builtin_fs_readFile(const std::vector<Value> &argvals, const nari::CallExpr*) {
    if (argvals.size() >= 1) {
...
Value ScriptRuntime::builtin_fs_writeFile(const std::vector<Value> &argvals, const nari::CallExpr*) {
    if (argvals.size() >= 2) {
        std::string path = argvals[0].to_string();
...
Value ScriptRuntime::builtin_fs_appendFile(const std::vector<Value> &argvals, const nari::CallExpr*) {
    if (argvals.size() >= 2) {
        std::string path = argvals[0].to_string();
        std::string content = argvals[1].to_string();
...
Value ScriptRuntime::builtin_fs_fileExists(const std::vector<Value> &argvals, const nari::CallExpr*) {
    if (argvals.size() >= 1) {
        std::string path = argvals[0].to_string();
...
Value ScriptRuntime::builtin_fs_isDirectory(const std::vector<Value> &argvals, const nari::CallExpr*) {
    if (argvals.size() >= 1) {
        std::string path = argvals[0].to_string();
        std::error_code ec;
...
Value ScriptRuntime::builtin_fs_deleteFile(const std::vector<Value> &argvals, const nari::CallExpr*) {
    if (argvals.size() >= 1) {
        std::string path = argvals[0].to_string();
...
Value ScriptRuntime::builtin_fs_listDir(const std::vector<Value> &argvals, const nari::CallExpr*) {
    if (argvals.size() >= 1) {
        std::string path = argvals[0].to_string();
...
// Network builtins
Value ScriptRuntime::builtin_net_createServer(const std::vector<Value> &argvals, const nari::CallExpr*) {
    if (argvals.size() >= 2) {
        int port = static_cast<int>(argvals[0].as_number());
        Value callback_val = argvals[1];
        std::string callback_name = callback_val.is_function() ? callback_val.get_function().name : "";
        ...
        if (io_pool) io_pool->submit(listen_op);
    }
    return Value::none();
}

Value ScriptRuntime::builtin_net_conn_read(const std::vector<Value> &argvals, const nari::CallExpr*) {
    if (argvals.size() >= 2 && argvals[0].is_object()) {
        auto& obj = argvals[0].get_object();
        if (!obj) return Value::none();
        ...
        if (fd_it != obj->end() && fd_it->second.is_int()) {
            ...
            read_op->callback = [this, callback_name, read_op]() {
                if (!callback_name.empty()) {
                    ...
                    if (it != functions.end()) {
                        if (read_op->success) {
                            call_user_function(it->second.get(), {Value::none(), Value::make_string(read_op->result_string)});
                        } else {
                            call_user_function(it->second.get(), {Value::make_string(read_op->error_msg), Value::none()});
                        }
                    }
                }
            };

            if (io_pool) io_pool->submit(read_op);
        }
    }
    return Value::none();
}

Value ScriptRuntime::builtin_net_conn_write(const std::vector<Value> &argvals, const nari::CallExpr*) {
                ...
                if (!callback_name.empty()) {
                    ...
                    if (it != functions.end()) {
                        if (write_op->success) {
                            ...
                            call_user_function(it->second.get(), {Value::make_string(write_op->error_msg)});
                        }
                    }
                }
            };

            if (io_pool) io_pool->submit(write_op);
        }
    }
    return Value::none();
}

Value ScriptRuntime::builtin_net_conn_close(const std::vector<Value> &argvals, const nari::CallExpr*) {
    if (argvals.size() >= 1 && argvals[0].is_object()) {
        ...
        if (fd_it != obj->end() && fd_it->second.is_int()) {
            ...
            if (io_pool) io_pool->submit(close_op);
        }
    }
    return Value::none();
}
...
#ifndef NO_OPENSSL
// HTTP builtins
Value ScriptRuntime::builtin_http_get(const std::vector<Value> &argvals, const nari::CallExpr*) {
    if (argvals.size() >= 1) {
        ...
        if (io_pool) io_pool->submit(http_op);
        return Value::make_handle(handle);
    }
    return Value::none();
}

Value ScriptRuntime::builtin_http_request(const std::vector<Value> &argvals, const nari::CallExpr*) {
    if (argvals.size() >= 2 && argvals[0].is_object()) {
        Value callback_val = argvals[1];
        std::string callback_name = callback_val.is_function() ? callback_val.get_function().name : "";

        auto& opts_ptr = argvals[0].get_object();
        if (!opts_ptr) return Value::none();
            ...
            if (!callback_name.empty()) {
                ...
                if (it != functions.end()) {
                    if (http_op->success) {
                        ...
                        call_user_function(it->second.get(), {Value::make_string(http_op->error_msg), Value::none()});
                    }
                }
            }
        };
...
// Array builtins
Value ScriptRuntime::builtin_push(const std::vector<Value> &argvals, const nari::CallExpr*) {
...
Value ScriptRuntime::builtin_pop(const std::vector<Value> &argvals, const nari::CallExpr*) {
...
Value ScriptRuntime::builtin_length(const std::vector<Value> &argvals, const nari::CallExpr*) {
    if (!argvals.empty()) {
...
Value ScriptRuntime::builtin_slice(const std::vector<Value> &argvals, const nari::CallExpr*) {
    if (!argvals.empty() && argvals[0].is_array()) {
        auto& arr_ptr = argvals[0].get_array();
        if (!arr_ptr) return Value::make_array();

        int start = 0;
        if (argvals.size() > 1) {
            if (!argvals[1].is_int()) return Value::make_array();
            start = argvals[1].get_int();
        }
        int end = arr_ptr->size();
        if (argvals.size() > 2) {
            if (!argvals[2].is_int()) return Value::make_array();
            end = argvals[2].get_int();
        }

        if (start < 0) start = 0;
        if (end > arr_ptr->size()) end = arr_ptr->size();
...
Value ScriptRuntime::builtin_concat(const std::vector<Value> &argvals, const nari::CallExpr*) {
    if (argvals.size() >= 2 &&
        argvals[0].is_array() &&
        argvals[1].is_array()) {
        auto& arr1 = argvals[0].get_array();
        auto& arr2 = argvals[1].get_array();
        if (!arr1 || !arr2) return Value::make_array();
...
// Object builtins
Value ScriptRuntime::builtin_keys(const std::vector<Value> &argvals, const nari::CallExpr*) {
    if (!argvals.empty() && argvals[0].is_object()) {
        auto& obj = argvals[0].get_object();
        if (!obj) return Value::make_array();
...
Value ScriptRuntime::builtin_values(const std::vector<Value> &argvals, const nari::CallExpr*) {
    if (!argvals.empty() && argvals[0].is_object()) {
        auto& obj = argvals[0].get_object();
        if (!obj) return Value::make_array();
...
Value ScriptRuntime::builtin_hasKey(const std::vector<Value> &argvals, const nari::CallExpr*) {
    if (argvals.size() >= 2 && argvals[0].is_object()) {
        auto& obj = argvals[0].get_object();
        if (!obj) return Value::make_bool(false);
...
Value ScriptRuntime::builtin_yield(const std::vector<Value>&, const nari::CallExpr*) {
    if (Runtime::g_shutdown_requested.load()) {
        if (io_pool) {
#ifndef NO_THREADS
...
Value ScriptRuntime::builtin_shutdown_requested(const std::vector<Value>&, const nari::CallExpr*) {
    return Value::make_bool(Runtime::g_shutdown_requested.load());
}

// String builtins
Value ScriptRuntime::builtin_substr(const std::vector<Value> &argvals, const nari::CallExpr*) {
    if (!argvals.empty()) {
        std::string str = argvals[0].to_string();
        int start = 0;
        if (argvals.size() > 1) {
            if (!argvals[1].is_int()) return Value::make_string("");
            start = argvals[1].get_int();
        }
        int len = str.size() - start;
        if (argvals.size() > 2) {
            if (!argvals[2].is_int()) return Value::make_string("");
            len = argvals[2].get_int();
        }

        if (start < 0) start = 0;
        if (start >= str.size()) return Value::make_string("");
        if (len < 0) len = 0;
...
Value ScriptRuntime::builtin_indexOf(const std::vector<Value> &argvals, const nari::CallExpr*) {
    if (argvals.size() >= 2) {
        std::string str = argvals[0].to_string();
...
Value ScriptRuntime::builtin_lastIndexOf(const std::vector<Value> &argvals, const nari::CallExpr*) {
    if (argvals.size() >= 2) {
        std::string str = argvals[0].to_string();
        std::string search = argvals[1].to_string();
        ...
        if (search.empty()) {
            if (argvals.size() > 2 && (argvals[2].is_int() || argvals[2].is_float())) {
                ...
                if (fi < 0) return Value::make_int(-1);
...
Value ScriptRuntime::builtin_split(const std::vector<Value> &argvals, const nari::CallExpr*) {
    if (argvals.size() >= 2) {
        std::string str = argvals[0].to_string();
...
Value ScriptRuntime::builtin_replace(const std::vector<Value> &argvals, const nari::CallExpr*) {
    if (argvals.size() >= 3) {
        std::string str = argvals[0].to_string();
    ...
    return argvals.empty() ? Value::make_string("") : Value::make_string(argvals[0].to_string());
}

Value ScriptRuntime::builtin_replaceAll(const std::vector<Value> &argvals, const nari::CallExpr*) {
    if (argvals.size() >= 3) {
        std::string str = argvals[0].to_string();
        std::string find = argvals[1].to_string();
        std::string replacement = argvals[2].to_string();
...
Value ScriptRuntime::builtin_trim(const std::vector<Value> &argvals, const nari::CallExpr*) {
    if (!argvals.empty()) {
        std::string str = argvals[0].to_string();
        size_t start = 0;
...
Value ScriptRuntime::builtin_toUpper(const std::vector<Value> &argvals, const nari::CallExpr*) {
    if (!argvals.empty()) {
        std::string str = argvals[0].to_string();
        for (char &c : str) {
            c = std::toupper(c);
...
Value ScriptRuntime::builtin_toLower(const std::vector<Value> &argvals, const nari::CallExpr*) {
    if (!argvals.empty()) {
        std::string str = argvals[0].to_string();
        for (char &c : str) {
            c = std::tolower(c);
...
Value ScriptRuntime::builtin_startsWith(const std::vector<Value> &argvals, const nari::CallExpr*) {
    if (argvals.size() >= 2) {
        std::string str = argvals[0].to_string();
...
Value ScriptRuntime::builtin_endsWith(const std::vector<Value> &argvals, const nari::CallExpr*) {
    if (argvals.size() >= 2) {
        std::string str = argvals[0].to_string();
...
Value ScriptRuntime::builtin_charAt(const std::vector<Value> &argvals, const nari::CallExpr*) {
    if (argvals.size() >= 2) {
        std::string str = argvals[0].to_string();
        if (!argvals[1].is_int()) return Value::make_string("");
...
Value ScriptRuntime::builtin_join(const std::vector<Value> &argvals, const nari::CallExpr*) {
    if (argvals.size() >= 2 && argvals[0].is_array()) {
        auto& arr = argvals[0].get_array();
        if (!arr) return Value::make_string("");
...
Value ScriptRuntime::builtin_typeof(const std::vector<Value> &argvals, const nari::CallExpr*) {
    if (!argvals.empty()) {
        if (argvals[0].is_none()) return Value::make_string("null");
        if (argvals[0].is_int()) return Value::make_string("int");
        if (argvals[0].is_float()) return Value::make_string("float");
        if (argvals[0].is_string()) return Value::make_string("string");
        if (argvals[0].is_bool()) return Value::make_string("bool");
        if (argvals[0].is_array()) return Value::make_string("array");
        if (argvals[0].is_object()) return Value::make_string("object");
        if (argvals[0].is_function()) return Value::make_string("function");
...
#ifndef NO_FFI
Value ScriptRuntime::builtin_ffi_membersof(const std::vector<Value> &argvals, const nari::CallExpr*) {
    if (argvals.empty()) {
    ...
    auto map_type_to_ffi = [](const std::string& nari_type) -> std::string {
        if (nari_type == "f32" || nari_type == "float") return "float";
        if (nari_type == "f64" || nari_type == "double") return "double"; 
        if (nari_type == "i8") return "i8";
        if (nari_type == "u8") return "u8";
        if (nari_type == "i16") return "i16";
        if (nari_type == "u16") return "u16";
        if (nari_type == "i32" || nari_type == "int") return "int";
        if (nari_type == "i64" || nari_type == "long") return "long";
        if (nari_type == "u32" || nari_type == "uint") return "uint";
        if (nari_type == "u64" || nari_type == "ulong") return "ulong";
        if (nari_type == "bool" || nari_type == "boolean") return "bool";
        if (nari_type == "string" || nari_type == "pointer") return "pointer";
    ...
    for (const auto& field : resolved_decl->fields) {
        ...
        (*field_obj)["type"] = Value::make_string(map_type_to_ffi(field.type->name));
...
Value ScriptRuntime::builtin_toNumber(const std::vector<Value> &argvals, const nari::CallExpr*) {
    if (!argvals.empty()) {
        const Value &v = argvals[0];
        if (v.is_int()) return Value::make_int(v.get_int());
        if (v.is_float()) return Value::make_float(v.get_float());
        if (v.is_bool()) return Value::make_int(v.get_bool() ? 1 : 0);
        if (v.is_string()) {
            ...
            if (is_float) {
                ...
                if (end && *end == '\0') return Value::make_float(dv);
                return Value::make_float(0.0);
            }
            errno = 0;
            char *end = nullptr;
            int64_t iv = std::strtoll(s.c_str(), &end, 10);
            if (end && *end == '\0' && errno != ERANGE) return Value::make_int(static_cast<int64_t>(iv));
...
Value ScriptRuntime::builtin_toString(const std::vector<Value> &argvals, const nari::CallExpr*) {
    if (!argvals.empty()) {
...
Value ScriptRuntime::builtin_toBool(const std::vector<Value> &argvals, const nari::CallExpr*) {
    if (!argvals.empty()) {
...
Value ScriptRuntime::builtin_isNumber(const std::vector<Value> &argvals, const nari::CallExpr*) {
    if (!argvals.empty()) {
...
Value ScriptRuntime::builtin_isString(const std::vector<Value> &argvals, const nari::CallExpr*) {
    if (!argvals.empty()) {
...
Value ScriptRuntime::builtin_isBool(const std::vector<Value> &argvals, const nari::CallExpr*) {
    if (!argvals.empty()) {
...
Value ScriptRuntime::builtin_isArray(const std::vector<Value> &argvals, const nari::CallExpr*) {
    if (!argvals.empty()) {
...
Value ScriptRuntime::builtin_isObject(const std::vector<Value> &argvals, const nari::CallExpr*) {
    if (!argvals.empty()) {
...
Value ScriptRuntime::builtin_isFunction(const std::vector<Value> &argvals, const nari::CallExpr*) {
    if (!argvals.empty()) {
...
// I/O builtins
Value ScriptRuntime::builtin_readLine(const std::vector<Value>&, const nari::CallExpr*) {
...
Value ScriptRuntime::builtin_readAll(const std::vector<Value>&, const nari::CallExpr*) {
    std::string result;
    char buf[4096];
    size_t n;
...
Value ScriptRuntime::builtin_time(const std::vector<Value>&, const nari::CallExpr*) {
...
#ifndef NO_FFI
// FFI builtins
Value ScriptRuntime::builtin_ffi_load_library(const std::vector<Value> &argvals, const nari::CallExpr*) {
    ...
    if (!lib || !lib->is_loaded()) {
        // return error object, later on this will be standardized into an Error type
        // to be used with either try/catch or Result<T, E>.
        auto err_obj = std::make_shared<std::unordered_map<std::string, Value>>();
        (*err_obj)["error"] = Value::make_string("Failed to load library: " + lib_path);
        if (lib) {
    ...
    (*lib_obj)["__ffi_handle__"] = Value::make_int(reinterpret_cast<int64_t>(lib.get()));

    const auto& symbols = lib->get_symbols();
    for (const auto& symbol : symbols) {
        // TODO: these are just placeholders, actual symbol values should be function pointers probably
...
Value ScriptRuntime::builtin_ffi_get_symbol(const std::vector<Value> &argvals, const nari::CallExpr*) {
    if (argvals.size() < 2 || !argvals[0].is_object() || !argvals[1].is_string()) {
        return Value::none();
    }
...
// used like: __ffi_call(loaded_lib_reference, "function_name", signature_obj, [args...])
// signature must look something like { return: "int", params: ["int", "string"] }
Value ScriptRuntime::builtin_ffi_call(const std::vector<Value> &argvals, const nari::CallExpr*) {


    if (argvals.size() < 3 || !argvals[0].is_object() || !argvals[1].is_string() || !argvals[2].is_object()) {
        return Value::none();
    }
    ...
    if (!argvals[0].is_object()) {
        fprintf(
            stderr,
            "ERROR: First argument to __ffi_call is not an object (type index: %zu)\n",
    ...
    auto parse_ffi_type = [&sig](const Value& type_val, bool is_return) -> FFIType {
        if (type_val.is_string()) {
            ...
            if (type_str == "void") return FFIType::Void;
            else if (type_str == "i8" || type_str == "int8" || type_str == "char") return FFIType::Int8;
            else if (type_str == "u8" || type_str == "uint8" || type_str == "uchar" || type_str == "byte") return FFIType::UInt8;
            else if (type_str == "i16" || type_str == "int16" || type_str == "short") return FFIType::Int16;
            else if (type_str == "u16" || type_str == "uint16" || type_str == "ushort" || type_str == "word") return FFIType::UInt16;
            else if (type_str == "int" || type_str == "i32" || type_str == "int32") return FFIType::Int32;
            else if (type_str == "long" || type_str == "i64" || type_str == "int64") return FFIType::Int64;
            else if (type_str == "uint" || type_str == "u32") return FFIType::UInt32;
            else if (type_str == "ulong" || type_str == "u64") return FFIType::UInt64;
            else if (type_str == "float") return FFIType::Float;
            else if (type_str == "double") return FFIType::Double;
            else if (type_str == "bool") return FFIType::Bool;
            else if (type_str == "string" || type_str == "pointer") return FFIType::Pointer;
                    ...
                    for (const auto& field_val : *fields_array) {
                        if (field_val.is_object()) {
                            ...
                            if (field_obj->find("name") != field_obj->end() &&
                                ...
                                if (field_type_str == "i8" || field_type_str == "int8" || field_type_str == "char") field_type = FFIType::Int8;
                                else if (field_type_str == "u8" || field_type_str == "uint8" || field_type_str == "uchar" || field_type_str == "byte") field_type = FFIType::UInt8;
                                else if (field_type_str == "i16" || field_type_str == "int16" || field_type_str == "short") field_type = FFIType::Int16;
                                else if (field_type_str == "u16" || field_type_str == "uint16" || field_type_str == "ushort" || field_type_str == "word") field_type = FFIType::UInt16;
                                else if (field_type_str == "int" || field_type_str == "i32" || field_type_str == "int32") field_type = FFIType::Int32;
                                else if (field_type_str == "long" || field_type_str == "i64" || field_type_str == "int64") field_type = FFIType::Int64;
                                else if (field_type_str == "uint" || field_type_str == "u32") field_type = FFIType::UInt32;
                                else if (field_type_str == "ulong" || field_type_str == "u64") field_type = FFIType::UInt64;
                                else if (field_type_str == "float") field_type = FFIType::Float;
                                else if (field_type_str == "double") field_type = FFIType::Double;
                                else if (field_type_str == "bool") field_type = FFIType::Bool;
    ...
    if (sig_obj->find("returns") != sig_obj->end()) {
        ...
        fprintf(stderr, "ERROR: FFI signature object is missing 'returns' field!\n");
        return Value::none();
    }
    ...
    // parse param types
    if (sig_obj->find("params") != sig_obj->end() && (*sig_obj)["params"].is_array()) {
    ...
    if (sig.is_variadic && func_args.size() > sig.fixed_param_count) {
        ...
        for (size_t i = sig.fixed_param_count; i < func_args.size(); i++) {
            ...
            if (arg.is_string()) {
                ...
                // realistically I don't think there will be a scenario where we ever get here, but default to int
                printf("Unable to infer FFI variadic argument type, defaulting to int32\n");
                sig.param_types.push_back(FFIType::Int32);
            }
        }
    }
...
Value ScriptRuntime::builtin_ffi_utf16(const std::vector<Value> &argvals, const nari::CallExpr*) {
    if (argvals.empty() || !argvals[0].is_string()) {
        return Value::make_int(0);
    }
...
Value ScriptRuntime::builtin_ffi_alloc(const std::vector<Value> &argvals, const nari::CallExpr*) {
    if (argvals.empty() || !argvals[0].is_int()) {
        return Value::make_int(0);
    }
...
Value ScriptRuntime::builtin_ffi_utf16_read(const std::vector<Value> &argvals, const nari::CallExpr*) {
    if (argvals.empty() || !argvals[0].is_int()) {
        return Value::make_string("");
    }
...
Value ScriptRuntime::builtin_ffi_free(const std::vector<Value> &argvals, const nari::CallExpr*) {
    if (argvals.empty() || !argvals[0].is_int()) {
        return Value::none();
    }
...
Value ScriptRuntime::builtin_ffi_alloc_struct(const std::vector<Value> &argvals, const nari::CallExpr*) {
    if (argvals.empty() || !argvals[0].is_string()) {
...
Value ScriptRuntime::builtin_ffi_read_struct(const std::vector<Value> &argvals, const nari::CallExpr*) {
...
Value ScriptRuntime::builtin_ffi_write_struct(const std::vector<Value> &argvals, const nari::CallExpr*) {
    if (argvals.size() < 3 || !argvals[0].is_int() || !argvals[1].is_string() || !argvals[2].is_object()) {
...
Value ScriptRuntime::builtin_ffi_sizeof(const std::vector<Value> &argvals, const nari::CallExpr*) {
    if (argvals.empty() || !argvals[0].is_string()) {
...
Value ScriptRuntime::builtin_platform_arch(const std::vector<Value>&, const nari::CallExpr*) {
...
Value ScriptRuntime::builtin_platform_os(const std::vector<Value>&, const nari::CallExpr*) {
    #if defined(__EMSCRIPTEN__)
...
Value ScriptRuntime::builtin_platform_endianness(const std::vector<Value>&, const nari::CallExpr*) {
    return std::endian::native == std::endian::little ? Value::make_string("little") : Value::make_string("big");
}

Value ScriptRuntime::builtin_platform_hostname(const std::vector<Value>&, const nari::CallExpr*) {
    char hostname[256];
    gethostname(hostname, sizeof(hostname));
    return Value::make_string(hostname);
}

Value ScriptRuntime::builtin_platform_getenv(const std::vector<Value>& argvals, const nari::CallExpr*) {
...
Value ScriptRuntime::builtin_system_exec(const std::vector<Value>& argvals, const nari::CallExpr*) {
    if (argvals.size() >= 1 && argvals[0].is_string()) {
...
Value ScriptRuntime::builtin_gc_collect(const std::vector<Value>&, const CallExpr*) {
...
Value ScriptRuntime::builtin_gc_stats(const std::vector<Value>&, const CallExpr*) {
    auto& gc = GarbageCollector::instance();
    auto stats = gc.get_stats();
...
Value ScriptRuntime::builtin_gc_enable(const std::vector<Value>& argvals, const CallExpr*) {
    auto& gc = GarbageCollector::instance();
...
#ifndef NO_FFI
// __ffi_create_callback(signature, nari_function) - create a native callback from a Nari function
Value ScriptRuntime::builtin_ffi_create_callback(const std::vector<Value> &argvals, const nari::CallExpr*) {
    if (argvals.size() < 2 || !argvals[0].is_object() || !argvals[1].is_function()) {
...
Value ScriptRuntime::builtin_ffi_free_callback(const std::vector<Value> &argvals, const nari::CallExpr*) {
    if (argvals.empty() || !argvals[0].is_int()) {
...
Value ScriptRuntime::builtin_gc_set_threshold(const std::vector<Value>& argvals, const CallExpr*) {
    auto& gc = GarbageCollector::instance();
