...
struct FileOperation : IOOperation {
  ...
  struct Result {
    ...
    Result(const Result &other) : type(other.type) { copy_from(other); }
    ...
    void move_from(Result &&other) {
      ...
      case Type::Array:
        new (&array_value)
            std::vector<std::string>(std::move(other.array_value));
        break;
      case Type::Bool:
        bool_value = other.bool_value;
        break;
...
#ifdef NO_THREADS
// synchronous stub for emscripten, run operations immediately without any
// threading
// TODO: this is kinda bad, I mostly did this only to make the emscripten build
...
public:
  ...
  void submit(IOOperationPtr op) {
    ...
    case IOOperation::Type::Timer:
      ...
      fprintf(stderr, "Warning: Timer ops are not supported in this build and "
                      "will be ignored!\n");
      op->success = false;
      break;
    ...
    case IOOperation::Type::ListDir:
      ...
      op->success = false;
      fprintf(stderr, "Warning: File I/O ops are not supported in this build "
                      "and will be ignored!\n");
      break;
  ...
  bool has_completed() { return !completed_queue.empty(); }
          ...
          if (op) {
            ...
            case IOOperation::Type::Timer:
              std::this_thread::sleep_for(
                  std::chrono::milliseconds(op->timer_ms));
              op->success = true;
              break;
            ...
            case IOOperation::Type::FileRead: {
              ...
              if (fp) {
                ...
                file_op->error_msg =
                    "Failed to open file: " + file_op->file_path;
              }
              break;
            }
            ...
            case IOOperation::Type::FileWrite: {
              ...
              if (fp) {
                fwrite(file_op->file_content.data(), 1,
                       file_op->file_content.size(), fp);
                fclose(fp);
                file_op->success = true;
              } else {
                file_op->success = false;
                file_op->error_msg =
                    "Failed to write file: " + file_op->file_path;
              }
              break;
            }
            ...
            case IOOperation::Type::FileAppend: {
              ...
              if (fp) {
                ...
                file_op->success = false;
                file_op->error_msg =
                    "Failed to append to file: " + file_op->file_path;
              }
              break;
            }
            ...
            case IOOperation::Type::FileExists: {
              auto file_op = std::static_pointer_cast<FileOperation>(op);
              file_op->result_bool =
                  std::filesystem::exists(file_op->file_path);
              file_op->success = true;
              break;
            }
            ...
            case IOOperation::Type::FileDelete: {
              ...
              std::error_code ec;
              file_op->result_bool =
                  std::filesystem::remove(file_op->file_path, ec);
              if (ec) {
                file_op->success = false;
            ...
            case IOOperation::Type::ListDir: {
              ...
              if (ec) {
                file_op->success = false;
                file_op->error_msg =
                    "Failed to list directory: " + ec.message();
              } else {
                // sort files, prioritize dot files/folders, and then
            ...
            case IOOperation::Type::TcpListen: {
              ...
              if (bind(sock_fd, (sockaddr *)&addr, sizeof(addr)) < 0) {
                tcp_op->success = false;
                tcp_op->error_msg = "Failed to bind to port " +
            ...
            case IOOperation::Type::TcpAccept: {
              ...
              int client_fd =
                  accept(tcp_op->socket_fd, (struct sockaddr *)&client_addr,
                         &client_len);
              if (client_fd < 0) {
                tcp_op->success = false;
                tcp_op->error_msg = "Failed to accept connection: " +
                                    std::string(strerror(errno));
                break;
              }
            ...
            case IOOperation::Type::TcpRead: {
              ...
              char buffer[8192];
              ssize_t bytes_read =
                  recv(tcp_op->socket_fd, buffer, sizeof(buffer) - 1, 0);

              if (bytes_read < 0) {
                tcp_op->success = false;
                tcp_op->error_msg = "Failed to read from socket: " +
                                    std::string(strerror(errno));
            ...
            case IOOperation::Type::TcpWrite: {
              ...
              while (sent < total) {
                ssize_t n =
                    send(tcp_op->socket_fd, data + sent, total - sent, 0);
                if (n < 0) {
                  tcp_op->success = false;
                  tcp_op->error_msg = "Failed to write to socket: " +
                                      std::string(strerror(errno));
                  break;
                }
                sent += n;
...
// so for now we just enable verification on linux only
                  ...
                  if (http_op->method == "GET") {
                    ...
                    res = client.Post(http_op->url_path, headers, http_op->body,
                ...
                if (res && res->status != -1) {
                  ...
                  auto err = res.error();
                  http_op->error_msg =
                      "HTTP request failed: " + httplib::to_string(err);
                }
              } catch (const std::exception &e) {
                http_op->success = false;
                http_op->error_msg =
                    "HTTP request exception: " + std::string(e.what());
              }
              break;
            }
#endif // ENABLE_HTTP
  ...
  bool has_pending() { return pending_count > 0 || has_completed(); }
