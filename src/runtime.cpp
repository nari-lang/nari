...
namespace Runtime {
    ...
    TraceLevel runtime_trace_level() { return static_cast<TraceLevel>(g_trace_level); }
...
static void runtime_fatal(const std::string &msg, const nari::ASTNode *node = nullptr) {
    std::string fname = "<runtime>";
    int l = 0, c = 0;
    if (node) {
        if (!node->filename.empty()) fname = node->filename;
        l = node->line;
        c = node->col;
    }
    ...
    for (const auto& funcname : toplevel_funcs) {
        ...
        if (f->function_expr && f->function_expr->body) {
            for (const auto &st : f->function_expr->body->stmts) {
                if (!st) continue;
                exec_stmt(st.get());
                if (flags.return_flag) break;
                if (flags.throw_flag) break;
            }
        } else if (f->body) {
...
void ScriptRuntime::step_task(HandlePtr handle) {
    ...
    if (task->state != Task::Running && task->state != Task::Yielded) return;
    ...
    case PatternKind::Variant: {
        ...
        if (!vp->fields.empty()) {
            ...
            if (data.is_array()) {
                ...
                if (!data_arr || vp->fields.size() != data_arr->size()) return false;
...
Value ScriptRuntime::eval_expr(const Expr *e) {
    ...
    case ExprKind::Ident: {
        ...
        if (ie->name == "true") return Value::make_bool(true);
        if (ie->name == "false") return Value::make_bool(false);

        for (int i = block_scope_stack.size() - 1; i >= 0; i--) {
            auto &block_scope = block_scope_stack[i];
            auto it = block_scope.find(ie->name);
            if (it != block_scope.end()) return it->second;
        }
        ...
        if (!module_stack.empty()) {
            ...
            if (it != module_local_vars.end()) {
                ...
                if (mlocal != it->second.end()) return mlocal->second;
            }
        }
        ...
        if (global_it != globals.end()) return global_it->second;
        ...
        // Undefined identifier: this is a fatal runtime error (do not coerce to string).
    ...
    case ExprKind::Number: {
        ...
        if (ne->is_float) return Value::make_float(ne->f);
    ...
    case ExprKind::Unary: {
        ...
        if (!unaryExpr->operand) return Value::none();
        const std::string &op = unaryExpr->op;

        // Prefix ++ and --
        // TODO: there is almost certainly a better way to do this, but i already spent like 2 hours on it and bleh
        if (op == "++" || op == "--") {
            if (!unaryExpr->operand) {
                runtime_fatal("Increment/decrement operand is null", unaryExpr);
            }
            const IdentExpr *ie = unaryExpr->operand->kind == ExprKind::Ident
                ? static_cast<const IdentExpr*>(unaryExpr->operand.get()) : nullptr;
            if (!ie) {
            ...
            if (!stored && !module_stack.empty()) {
                auto it = module_local_vars.find(module_stack.back());
                if (it != module_local_vars.end() && it->second.find(ie->name) != it->second.end()) {
    ...
    case ExprKind::Binary: {
        ...
        if (op == "&&") {
            if (!binaryExpr->left || !binaryExpr->right) return Value::none();
            Value a = eval_expr(binaryExpr->left.get());
            if (!a.as_bool()) return Value::make_bool(false);
        ...
        if (op == "||") {
            if (!binaryExpr->left || !binaryExpr->right) return Value::none();
            Value a = eval_expr(binaryExpr->left.get());
            if (a.as_bool()) return Value::make_bool(true);
            Value b = eval_expr(binaryExpr->right.get());
            return Value::make_bool(b.as_bool());
        }
        if (op == "??") {
        ...
        if (op == "/") {
            ...
            if (rn == 0.0) return Value::make_float(0.0);
            return Value::make_float(left.as_number() / rn);
        }
        if (op == "%") {
            if (left.is_int() && right.is_int()) {
                if (right.get_int() == 0) return Value::make_int(0);
                return Value::make_int(left.get_int() % right.get_int());
            }
            double rn = right.as_number();
            if (rn == 0.0) return Value::make_float(0.0);
        ...
        if (op == "**") {
            if (left.is_int() && right.is_int() && right.get_int() >= 0) {
                ...
                while (exp > 0) {
                    if (exp & 1ULL) {
                        __int128 v = static_cast<__int128>(result) * static_cast<__int128>(base);
                    ...
                    if (exp > 1) {
                        __int128 v = static_cast<__int128>(base) * static_cast<__int128>(base);
                        if (v < std::numeric_limits<int64_t>::min() || v > std::numeric_limits<int64_t>::max()) {
                            return Value::make_float(std::pow(left.as_number(), right.as_number()));
                        }
        ...
        if (op == "&") {
            int64_t l = left.is_int() ? left.get_int() : static_cast<int64_t>(left.as_number());
            int64_t r = right.is_int() ? right.get_int() : static_cast<int64_t>(right.as_number());
    ...
    case ExprKind::Call: {
        ...
        if (callExpr->callee->kind == ExprKind::Member) {
            ...
            if (obj.is_class_instance()) {
                ...
                if (method->visibility == nari::Visibility::Private) {
                    if (current_class_name != instance->class_name) {
                        runtime_fatal("Cannot call private method '" + method_name + 
                                    "' of class " + instance->class_name, callExpr);
                    }
                }
                ...
                if (arg_values.size() != method->params.size()) {
                    runtime_fatal("Method '" + method_name + "' expects " + std::to_string(method->params.size()) + 
                                " arguments but got " + std::to_string(arg_values.size()), callExpr);
                }
            ...
            // First check if the object has this member as a property (for obj.method() on objects)
            if (obj.is_object()) {
            ...
            // If object doesn't have this as a property, check if it's a builtin method
        ...
        for (const auto &a : callExpr->args) argvals.push_back(eval_expr(a.get()));
    ...
    case ExprKind::StringInterpolation: {
        ...
        for (size_t i = 0; i < sie->parts.size(); ++i) {
            ...
            if (i < sie->expr_sources.size()) {
                ...
                std::string saved_filename = !sie->filename.empty() ? sie->filename : "<interpolation>";
                Parser::set_source_filename(saved_filename);

                auto expr_funcs = Parser::parse_program_from_source(sie->expr_sources[i]);
                Parser::set_source_filename(saved_filename);
                ...
                if (expr_funcs.size() < 2 || !expr_funcs[1] || !expr_funcs[1]->body ||
                    ...
                    runtime_fatal("Failed to parse interpolated expression: " + sie->expr_sources[i], sie);
                }

                // Extract and evaluate the expression from the per-module function
                auto *first_stmt = expr_funcs[1]->body->stmts[0].get();
                auto *expr_stmt = first_stmt->stmt_kind == StmtKind::Expr ? (nari::ExprStmt*)first_stmt : nullptr;
                if (!expr_stmt || !expr_stmt->expr) {
                    runtime_fatal("String interpolation must contain expressions: " + sie->expr_sources[i], sie);
                }
    ...
    case ExprKind::Function: {
        ...
        if (!call_stack.empty()) {
            if (!current_scope_closure) {
                ...
                current_scope_closure = std::make_shared<std::unordered_map<std::string, Value>>(call_stack.back());
            }
            func->closure_env_ptr = new std::shared_ptr<std::unordered_map<std::string, Value>>(current_scope_closure);
            // set up deleter for proper cleanup
            func->closure_deleter = [](void* ptr) {
                delete static_cast<std::shared_ptr<std::unordered_map<std::string, Value>>*>(ptr);
            };
        }
    ...
    case ExprKind::Index: {
        ...
        if (obj.is_array()) {
            ...
            if (!arr || idx < 0 || idx >= static_cast<int64_t>(arr->size())) {
                std::string error_msg = "Array index out of bounds: " + std::to_string(idx) + " (size: " + std::to_string(arr ? arr->size() : 0) + ")";
                    ...
                    if (field.visibility == nari::Visibility::Private) {
                        ...
                        if (current_class_name != instance->class_name) {
                            runtime_fatal("Cannot access private field '" + me->member + 
                                        "' of class " + instance->class_name, me);
                        }
                    }
    ...
    case ExprKind::New: {
        ...
        const nari::ClassDecl* class_decl = Parser::get_registered_class(ne->class_name);
        ...
        nari::ClassMethod* constructor = const_cast<nari::ClassDecl*>(class_decl)->get_constructor();
        if (constructor) {
            ...
            if (arg_values.size() != constructor->params.size()) {
                runtime_fatal("Constructor expects " + std::to_string(constructor->params.size()) + 
                            " arguments but got " + std::to_string(arg_values.size()), ne);
            }
            ...
            if (constructor->body) {
                for (const auto& stmt : constructor->body->stmts) {
                    exec_stmt(stmt.get());
                    if (flags.return_flag || flags.break_flag || flags.continue_flag || flags.throw_flag) {
                        break;
                    }
                }
            }
            ...
            if (flags.return_flag) flags.return_flag = false;
        } else if (!ne->args.empty()) {
            runtime_fatal("Class " + ne->class_name + " has no constructor but arguments were provided", ne);
        }
...
void ScriptRuntime::exec_stmt(const Stmt *s) {
    ...
    case StmtKind::VarDecl: {
        ...
        if (varDecl->initializerExpr) val = eval_expr(varDecl->initializerExpr.get());
    ...
    case StmtKind::IndexAssign: {
        ...
        if (indexAssignStmt->target->kind == ExprKind::Index) {
            const auto *ie = static_cast<const IndexExpr*>(indexAssignStmt->target.get());
            Value obj = eval_expr(ie->object.get());
            Value index = eval_expr(ie->index.get());
        ...
        // handle MemberExpr target: obj.member = val
            ...
            const auto *me = static_cast<const MemberExpr*>(indexAssignStmt->target.get());
            Value obj = eval_expr(me->object.get());
            ...
            if (obj.is_class_instance()) {
                ...
                for (const auto& field : class_decl->fields) {
                    if (field.name == me->member) {
                        ...
                        if (field.visibility == nari::Visibility::Private) {
                            if (current_class_name != instance->class_name) {
                                runtime_fatal("Cannot assign to private field '" + me->member +  "' of class " + instance->class_name, indexAssignStmt);
                            }
                        }
                        found = true;
                        break;
                ...
                if (!found) {
                    runtime_fatal("Class " + instance->class_name + " has no field '" + me->member + "'", indexAssignStmt);
                }
    ...
    case StmtKind::If: {
        ...
        bool take = false;
        if (ifStmt->cond) take = eval_expr(ifStmt->cond.get()).as_bool();
        if (take) {
            if (ifStmt->then_branch) exec_stmt(ifStmt->then_branch.get());
        } else {
            if (ifStmt->else_branch) exec_stmt(ifStmt->else_branch.get());
        }
        return;
    }
    case StmtKind::While: {
        ...
        while (true) {
            bool ok = true;
            if (whileStmt->cond) ok = eval_expr(whileStmt->cond.get()).as_bool();
            if (!ok) break;
            // clear continue flag at loop top
            flags.continue_flag = false;
            exec_stmt(whileStmt->body.get());
            if (flags.return_flag) return;
            if (flags.throw_flag) return;
            if (flags.shutdown_flag) return;
            // check for shutdown and break
            if (Runtime::g_shutdown_requested.load()) break;
            if (Runtime::g_runtime_error_occurred.load()) break;
            if (flags.break_flag) {
            ...
            if (values_equal(target, match)) {
                ...
                for (size_t j = i; j < switchStmt->cases.size(); j++) {
                    if (!is_empty_body(switchStmt->cases[j].body)) {
                        ...
                        if (flags.break_flag) flags.break_flag = false;
                        return;
                    }
                }
    ...
    case StmtKind::For: {
        ...
        if (forStmt->init) exec_stmt(forStmt->init.get());

        while (true) {
            bool ok = true;
            if (forStmt->cond) ok = eval_expr(forStmt->cond.get()).as_bool();
            if (!ok) break;
            // clear continue flag at loop top
            flags.continue_flag = false;
            ...
            if (flags.continue_flag) {
                ...
                if (forStmt->post) exec_stmt(forStmt->post.get());
                continue;
            }
...
Value ScriptRuntime::call_user_function(Function *func, const std::vector<Value> &args) {
    ...
    if (Runtime::runtime_trace_enabled()) {
        std::string trace_msg = "enter function: " + func->name + " @ " + func->filename + ":" + std::to_string(func->line) + ":" + std::to_string(func->col);
        Runtime::runtime_log(Runtime::TraceLevel::Debug, trace_msg);
    }
    ...
    if (func->closure_env_ptr) {
        closure_env_ref = *static_cast<std::shared_ptr<std::unordered_map<std::string, Value>>*>(func->closure_env_ptr);
    ...
    if (func->function_expr && func->function_expr->body) {
        ...
        for (const auto &st : func->function_expr->body->stmts) {
            ...
            if (flags.break_flag) flags.break_flag = false;
            if (flags.continue_flag) flags.continue_flag = false;
        }
    } else if (func->body) {
    ...
    if (closure_env_ref) {
        ...
        // and spent 4 hours debugging why the callstack would get corrupted and explode
...
void run_program_with_runtime(std::vector<std::unique_ptr<nari::Function>> &funcs) {
    ...
    // this means user code can override stdlib names, but i will be sad if you do that.
    std::vector<std::unique_ptr<nari::Function>> combined;
    combined.reserve(stdlib_funcs.size() + funcs.size());
    for (auto &f : stdlib_funcs) combined.push_back(std::move(f));
    for (auto &f : funcs) combined.push_back(std::move(f));

    try {
        ScriptRuntime rt(combined);
        rt.run_top_level();
    } catch (const RuntimeError &err) {
        // runtime error occurred, at this point error details have already been printed to stderr
        // just rethrow to let caller decide what to do
        throw;
    }
}
