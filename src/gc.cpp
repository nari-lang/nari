...
void GarbageCollector::clear_marks() { marked_objects.clear(); }
...
void GarbageCollector::mark_array(
    const std::shared_ptr<std::vector<Value>> &arr) {
  if (!arr)
    return;
...
void GarbageCollector::mark_object(
...
size_t GarbageCollector::sweep() {
  ...
  for (auto &member : cycle_members) {
    ...
    case TrackedType::Array: {
      auto arr =
          std::static_pointer_cast<std::vector<Value>>(member.locked_ptr);
      arr->clear();
      break;
    }
    case TrackedType::Object: {
      auto obj =
          std::static_pointer_cast<std::unordered_map<std::string, Value>>(
              member.locked_ptr);
      obj->clear();
      break;
    }
  ...
  // shrink the tracking map if it's significantly smaller than its bucket
  // count. this frees the oversized hash table from peak allocation periods.
  if (tracked_objects.bucket_count() > 4 * (tracked_objects.size() + 1)) {
    tracked_objects.rehash(tracked_objects.size());
  }
...
size_t
GarbageCollector::force_collect(const std::vector<const Value *> &roots) {
...
GarbageCollector::Stats GarbageCollector::get_stats() const {
  std::lock_guard<std::mutex> lock(mutex);
  return Stats{tracked_objects.size(), allocation_count, total_collections,
               total_collected,        total_allocated,  peak_tracked,
               collection_threshold,   refcount_freed,   enabled};
}
