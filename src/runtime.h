...
inline void reset_runtime_error_flag() { g_runtime_error_occurred.store(false); }
...
private:
    ...
    using BuiltinFn = Value (ScriptRuntime::*)(const std::vector<Value>&, const CallExpr*);

    static const std::unordered_map<std::string, BuiltinFn>& get_global_builtin_table() {
    ...
    static const std::unordered_map<std::string, BuiltinFn>& get_method_builtin_table() {
        static const std::unordered_map<std::string, BuiltinFn> table = {
    ...
    // which is an x-macro set of method names for that type, like {"substr", &ScriptRuntime::builtin_substr}, etc.
    ...
    Value call_builtin(const std::string &name, const std::vector<Value> &argvals, const CallExpr *ce) {
        auto &globals_tbl = get_global_builtin_table();
        auto it = globals_tbl.find(name);
        if (it != globals_tbl.end()) return (this->*(it->second))(argvals, ce);

        auto &methods_tbl = get_method_builtin_table();
        auto it2 = methods_tbl.find(name);
        if (it2 != methods_tbl.end()) return (this->*(it2->second))(argvals, ce);
    ...
    std::unordered_map<std::string, std::unordered_map<std::string, Value>> module_local_vars;
    ...
    bool has_pending_io() {
        if (!io_pool) return !active_intervals.empty();
    ...
    Value builtin_setInterval(const std::vector<Value> &argvals, const CallExpr*);
    Value builtin_clearInterval(const std::vector<Value> &argvals, const CallExpr*);
    ...
    Value builtin_fs_readFile(const std::vector<Value> &argvals, const CallExpr*);
    Value builtin_fs_writeFile(const std::vector<Value> &argvals, const CallExpr*);
    Value builtin_fs_appendFile(const std::vector<Value> &argvals, const CallExpr*);
    Value builtin_fs_fileExists(const std::vector<Value> &argvals, const CallExpr*);
    Value builtin_fs_isDirectory(const std::vector<Value> &argvals, const CallExpr*);
    Value builtin_fs_deleteFile(const std::vector<Value> &argvals, const CallExpr*);
    Value builtin_fs_listDir(const std::vector<Value> &argvals, const CallExpr*);
    Value builtin_net_createServer(const std::vector<Value> &argvals, const CallExpr*);
    Value builtin_net_conn_read(const std::vector<Value> &argvals, const CallExpr*);
    Value builtin_net_conn_write(const std::vector<Value> &argvals, const CallExpr*);
    Value builtin_net_conn_close(const std::vector<Value> &argvals, const CallExpr*);
#ifndef NO_OPENSSL
    Value builtin_http_get(const std::vector<Value> &argvals, const CallExpr*);
    Value builtin_http_request(const std::vector<Value> &argvals, const CallExpr*);
#endif
    ...
    Value builtin_shutdown_requested(const std::vector<Value>&, const CallExpr*);
    ...
    Value builtin_lastIndexOf(const std::vector<Value> &argvals, const CallExpr*);
...
#ifndef NO_FFI
    Value builtin_ffi_load_library(const std::vector<Value> &argvals, const CallExpr*);
    Value builtin_ffi_get_symbol(const std::vector<Value> &argvals, const CallExpr*);
    Value builtin_ffi_call(const std::vector<Value> &argvals, const CallExpr*);
    Value builtin_ffi_membersof(const std::vector<Value> &argvals, const CallExpr*);
    Value builtin_ffi_alloc(const std::vector<Value> &argvals, const CallExpr*);
    Value builtin_ffi_alloc_struct(const std::vector<Value> &argvals, const CallExpr*);
    Value builtin_ffi_read_struct(const std::vector<Value> &argvals, const CallExpr*);
    Value builtin_ffi_write_struct(const std::vector<Value> &argvals, const CallExpr*);
    ...
    Value builtin_ffi_utf16_read(const std::vector<Value> &argvals, const CallExpr*);
    Value builtin_ffi_free(const std::vector<Value> &argvals, const CallExpr*);
    Value builtin_ffi_create_callback(const std::vector<Value> &argvals, const CallExpr*);
    Value builtin_ffi_free_callback(const std::vector<Value> &argvals, const CallExpr*);
#endif
    ...
    Value builtin_platform_endianness(const std::vector<Value>&, const CallExpr*);
