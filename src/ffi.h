...
struct FFIStructDef {
    ...
    FFIStructDef& operator=(FFIStructDef&& other) noexcept {
        if (this != &other) {
            name = std::move(other.name);
            fields = std::move(other.fields);
            if (ffi_struct_type) delete ffi_struct_type;
            ffi_struct_type = other.ffi_struct_type;
            ffi_field_types = std::move(other.ffi_field_types);
            other.ffi_struct_type = nullptr;
        }
...
private:
    ...
    static std::unordered_map<FFISignature, CIFCache, FFISignatureHash> cif_cache_;
    static std::unordered_map<FFISignature, CIFCache, FFISignatureHash> cif_variadic_cache_;
