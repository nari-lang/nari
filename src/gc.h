...
public:
    ...
    // some simple statistics, currently the script can call these to get information
    // but i'm not sure if I want to keep it like that, since a script knowing about gc internals could be bad
    ...
    bool should_collect() const {
        return this->trigger_collection;
    }
...
private:
    ...
    void mark_object(const std::shared_ptr<std::unordered_map<std::string, Value>>& obj);
