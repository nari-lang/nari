/*
  parser.cpp
  Expression-precedence parser (precedence-climbing) that builds the AST for expressions.
...
static std::string token_desc(const Token &tok) {
    if (tok.text.empty()) return "<empty>";
    return tok.text;
}
...
void set_source_filename(const std::string &fn) {
...
static std::string resolve_include_path(const std::string &inc, const std::string &basefile) {
    ...
    if (inc.empty()) return inc;

    fs::path incp = inc;
    if (incp.is_absolute()) return incp.lexically_normal().string();
    ...
    if (basefile.empty()) {
        ...
        fs::path bf(basefile);
        if (bf.has_parent_path()) basepath = bf.parent_path();
        else basepath = fs::current_path();
    }
...
void pop_error_context() {
    if (!g_error_contexts.empty()) g_error_contexts.pop_back();
}
...
static void fatal_error(const std::string &msg, const Token *tok = nullptr) {
    std::string fname = g_current_filename.empty() ? std::string("<input>") : g_current_filename;
    int l = 0, c = 0;
    if (tok) {
        if (!tok->filename.empty()) fname = tok->filename;
        l = tok->line;
        c = tok->col;
    }
    ...
    auto print_source_context = [&](const std::string &file, int line, int col) {
        if (file.empty() || file[0] == '<' || line <= 0) return;
...
std::vector<FunctionPtr> parse_program_from_source(const std::string &src, bool create_aggregator = true);
...
public:
    ...
    std::vector<FunctionPtr> parse_program(bool create_aggregator = true) {
        ...
        while (!is_eof()) {
            ...
            if (tok.kind == TokenKind::TK_IDENT && tok.text == "import") {
                ...
                if (peek().kind == TokenKind::TK_IDENT) {
                    ...
                    if (peek().kind == TokenKind::TK_SEMICOLON) next();
                    continue;
                }
                ...
                std::string base_for_resolve = !tok.filename.empty() ? tok.filename : g_current_filename;
                ...
                if (g_visited_files.find(inc_path) == g_visited_files.end()) {
                    ...
                    std::string context = (
                        g_current_filename.empty()
                            ? std::string("<input>")
                            : g_current_filename) + ":" + std::to_string(tok.line) + ":" + std::to_string(tok.col);
                    ...
                    // remember previous filename and tell parser about the included filename
                    std::string prev_file = g_current_filename;
                    set_source_filename(inc_path);

                    // parse the included module and merge its functions into current vector
                    auto more_funcs = parse_program_from_source(included_src, false);
                    for (auto &f : more_funcs) functions.push_back(std::move(f));
        ...
        if (!top_block->stmts.empty()) {
            ...
            std::string modname = g_current_filename.empty() ? std::string("<input>") : g_current_filename;
            ...
            if (create_aggregator) {
                // build an aggregator __top_level__ function that will call all per-module
                // synthetic top functions discovered in this module's function list.
...
private:
    ...
    bool match(TokenKind k) {
        if (!is_eof() && peek().kind == k) { next(); return true; }
        return false;
    }

    // TODO: This needs finer grained control over exit condition
    // i.e whether we should return control back to the runtime, which can then do whatever, or actually exit.
    void error_and_exit(const std::string &msg) {
        const Token *tok = nullptr;
        if (!is_eof()) tok = &peek();

        std::string use_fname = g_current_filename.empty() ? std::string("<input>") : g_current_filename;
        int line = 0;
        int col = 0;

        if (tok) {
            // If the lexer attached a filename to the token, prefer it.
            if (!tok->filename.empty()) use_fname = tok->filename;
            line = tok->line;
            col = tok->col;
        }
        ...
        auto print_source_context = [&](const std::string &file, int lno, int cno) {
            if (file.empty() || file[0] == '<' || lno <= 0) return;
            FILE* fp = fopen(file.c_str(), "r");
            if (!fp) return;
    ...
    void expect(TokenKind k, const char *msg = nullptr) {
        if (!is_eof() && peek().kind == k) { next(); return; }
        std::string error_msg = "Parse error";
        if (!is_eof()) error_msg += " token='" + token_desc(peek()) + "'";
    ...
    bool looks_like_func_keyword_decl() {
        // func IDENT '(' ... ')' [-> type] '{'
        if (peek().kind != TokenKind::TK_IDENT || peek().text != "func") return false;
        if (peek(1).kind != TokenKind::TK_IDENT) return false;
        if (peek(2).kind != TokenKind::TK_LPAREN) return false;
        // find matching RPAREN
        size_t i = idx + 3;
        int depth = 1;
        while (i < toks.size()) {
            if (toks[i].kind == TokenKind::TK_LPAREN) ++depth;
            else if (toks[i].kind == TokenKind::TK_RPAREN) { --depth; if (depth == 0) break; }
            else if (toks[i].kind == TokenKind::TK_EOF) break;
            ++i;
        }
        ...
        if (next_idx < toks.size() && toks[next_idx].kind == TokenKind::TK_LBRACE) return true;
        return false;
    }

    bool looks_like_foreach_loop() {
        // for ( IDENT in ... )
        if (peek().kind != TokenKind::TK_IDENT || peek().text != "for") return false;
        if (peek(1).kind != TokenKind::TK_LPAREN) return false;
        if (peek(2).kind != TokenKind::TK_IDENT) return false;
        if (peek(3).kind != TokenKind::TK_IDENT || peek(3).text != "in") return false;
        return true;
    }

    bool looks_like_type_decl() {
        // type IDENT { or type IDENT< (generics) or type IDENT IDENT (alias)
        if (peek().kind != TokenKind::TK_IDENT || peek().text != "type") return false;
        if (peek(1).kind != TokenKind::TK_IDENT) return false;
        // Could be type Name { or type Name<T> { or type Alias BaseType
        if (peek(2).kind == TokenKind::TK_LBRACE || peek(2).kind == TokenKind::TK_LT || peek(2).kind == TokenKind::TK_IDENT) return true;
        return false;
    }

    bool looks_like_enum_decl() {
        // enum IDENT { or enum IDENT< (generics)
        if (peek().kind != TokenKind::TK_IDENT || peek().text != "enum") return false;
        if (peek(1).kind != TokenKind::TK_IDENT) return false;
        if (peek(2).kind == TokenKind::TK_LBRACE || peek(2).kind == TokenKind::TK_LT) return true;
        return false;
    }

    bool looks_like_class_decl() {
        // class IDENT { or class IDENT< (generics)
        if (peek().kind != TokenKind::TK_IDENT || peek().text != "class") return false;
        if (peek(1).kind != TokenKind::TK_IDENT) return false;
        if (peek(2).kind == TokenKind::TK_LBRACE || peek(2).kind == TokenKind::TK_LT) return true;
        return false;
    }
    ...
    nari::TypeDeclPtr parse_type_decl() {
        ...
        type_decl->filename = nameTok.filename.empty() ? g_current_filename : nameTok.filename;
    ...
    nari::EnumDeclPtr parse_enum_decl() {
        ...
        enum_decl->filename = nameTok.filename.empty() ? g_current_filename : nameTok.filename;
    ...
    nari::ClassDeclPtr parse_class_decl() {
        ...
        class_decl->filename = nameTok.filename.empty() ? g_current_filename : nameTok.filename;
        ...
        if (peek().kind != TokenKind::TK_RPAREN) {
            while (true) {
                ...
                if (peek().kind == TokenKind::TK_ELLIPSIS) {
                    if (seen_rest) error_and_exit("Only one rest parameter is allowed");
                    seen_rest = true;
                    is_rest = true;
                    next(); // consume '...'
                }
                ...
                if (peek().kind == TokenKind::TK_COMMA) next();
                else break;
            }
        }
    ...
    BlockPtr parse_block() {
        ...
        while (!is_eof() && peek().kind != TokenKind::TK_RBRACE) {
            auto stmt = parse_stmt();
            if (stmt) blk->stmts.push_back(std::move(stmt));
            else break;
        }
    ...
    bool get_loop_init(nari::Stmt *init, std::string &var, int64_t &start) {
        if (auto *vd = dynamic_cast<nari::VarDeclStmt*>(init)) {
            if (!vd->initializerExpr) return false;
            int64_t v = 0;
            if (!literal_int(vd->initializerExpr.get(), v)) return false;
            var = vd->name;
            start = v;
            return true;
        }
        if (auto *as = dynamic_cast<nari::AssignStmt*>(init)) {
            if (!as->value) return false;
            int64_t v = 0;
            if (!literal_int(as->value.get(), v)) return false;
            var = as->target;
            start = v;
            return true;
        }
    ...
    bool get_loop_cond(nari::Expr *cond, const std::string &var, int64_t &end, bool &inclusive) {
        ...
        if (!be || !be->left || !be->right) return false;
        if (be->op != "<" && be->op != "<=") return false;
        auto *id = dynamic_cast<nari::IdentExpr*>(be->left.get());
        if (!id || id->name != var) return false;
        int64_t v = 0;
        if (!literal_int(be->right.get(), v)) return false;
        inclusive = (be->op == "<=");
        end = v;
        return true;
    }
    ...
    bool get_loop_step(nari::Stmt *post, const std::string &var, int64_t &step) {
        ...
        if (auto *as = dynamic_cast<nari::AssignStmt*>(post)) {
            if (as->target != var || !as->value) return false;
        ...
        if (auto *es = dynamic_cast<nari::ExprStmt*>(post)) {
            ...
            if (!id || id->name != var) return false;
            if (ue->op == "post++") { step = 1; return true; }
            if (ue->op == "post--") { step = -1; return true; }
        }
        return false;
    }
    ...
    bool get_body_increment(nari::Stmt *body, std::string &target, int64_t &inc) {
        ...
        if (!blk || blk->stmts.size() != 1) return false;

        if (auto *as = dynamic_cast<nari::AssignStmt*>(blk->stmts[0].get())) {
            auto *be = dynamic_cast<nari::BinaryExpr*>(as->value.get());
            if (!be || !be->left || !be->right) return false;
            auto *id = dynamic_cast<nari::IdentExpr*>(be->left.get());
            if (!id || id->name != as->target) return false;
            int64_t v = 0;
            if (!literal_int(be->right.get(), v)) return false;
            if (be->op == "+") { target = as->target; inc = v; return true; }
            if (be->op == "-") { target = as->target; inc = -v; return true; }
            return false;
        }
        ...
        if (auto *es = dynamic_cast<nari::ExprStmt*>(blk->stmts[0].get())) {
            ...
            auto *id = dynamic_cast<nari::IdentExpr*>(ue->operand.get());
            if (!id) return false;
            if (ue->op == "post++") { target = id->name; inc = 1; return true; }
            if (ue->op == "post--") { target = id->name; inc = -1; return true; }
        }
    ...
    std::unique_ptr<nari::Stmt> try_optimize_for(nari::ForStmt *fs, nari::Stmt *prev) {
        if (!fs || !fs->init || !fs->cond || !fs->post || !fs->body) return nullptr;

        std::string loop_var;
        int64_t start = 0;
        if (!get_loop_init(fs->init.get(), loop_var, start)) return nullptr;

        int64_t end = 0;
        bool inclusive = false;
        if (!get_loop_cond(fs->cond.get(), loop_var, end, inclusive)) return nullptr;

        int64_t step = 0;
        if (!get_loop_step(fs->post.get(), loop_var, step)) return nullptr;
        ...
        std::string target;
        int64_t inc = 0;
        if (!get_body_increment(fs->body.get(), target, inc)) return nullptr;
        if (target == loop_var) return nullptr;
        ...
        if (!prev_numeric) return nullptr;

        __int128 diff = static_cast<__int128>(end) - static_cast<__int128>(start);
        if (inclusive) diff += 1;
        if (diff <= 0) diff = 0;
        __int128 delta128 = diff * static_cast<__int128>(inc);
        if (delta128 < std::numeric_limits<int64_t>::min() || delta128 > std::numeric_limits<int64_t>::max()) {
            return nullptr;
        }
    ...
    static std::string literal_to_string(const nari::Expr *e) {
        if (const auto *se = dynamic_cast<const nari::StringExpr*>(e)) return se->value;
        if (const auto *be = dynamic_cast<const nari::BoolExpr*>(e)) return be->value ? "true" : "false";
        if (const auto *ne = dynamic_cast<const nari::NumberExpr*>(e)) {
            if (ne->is_float) return std::to_string(ne->f);
            else return std::to_string(ne->i);
        }
        return "";
    }
    ...
    ExprPtr fold_expr(ExprPtr expr) {
        ...
        if (auto *binaryExpr = dynamic_cast<nari::BinaryExpr*>(expr.get())) {
            ...
            if (!binaryExpr->left || !binaryExpr->right) return expr;
            ...
            if (lint && rint) {
                if (op == "+") {
                    ...
                    if (v < std::numeric_limits<int64_t>::min() || v > std::numeric_limits<int64_t>::max()) return expr;
                ...
                if (op == "/") {
                    double val = (ri == 0) ? 0.0 : (static_cast<double>(li) / static_cast<double>(ri));
                ...
                if (op == "**") {
                    ...
                    if (ri > std::numeric_limits<uint64_t>::max()) return expr;
            ...
            if (op == "@" && literal_any(binaryExpr->left.get()) && literal_any(binaryExpr->right.get()) && (lstr || rstr)) {
            ...
            if (op == "==" || op == "!=") {
                ...
                if (lint && rint) {
                    ...
                    double l = lfloat ? lf : (lint ? static_cast<double>(li) : number_to_double(dynamic_cast<const nari::NumberExpr*>(binaryExpr->left.get())));
                    double r = rfloat ? rf : (rint ? static_cast<double>(ri) : number_to_double(dynamic_cast<const nari::NumberExpr*>(binaryExpr->right.get())));
    ...
    bool looks_like_arrow_function() {
        if (peek().kind != TokenKind::TK_LPAREN) return false;
        ...
        while (i < toks.size() && depth > 0) {
            if (toks[i].kind == TokenKind::TK_LPAREN) ++depth;
            else if (toks[i].kind == TokenKind::TK_RPAREN) --depth;
            else if (toks[i].kind == TokenKind::TK_EOF) return false;
            ++i;
        }

        // Check if next two tokens are => (we don't have a single => token, so check for =>, or in this case we need to add it to TokenKind)
        // For now, let's implement without arrow functions since we'd need to add the => token
        return false;
    }
    ...
    ExprPtr parse_function_expression() {
        ...
        func_expr->filename = funcTok.filename.empty() ? g_current_filename : funcTok.filename;
    ...
    ExprPtr parse_spawn_expression() {
        ...
        spawn_expr->filename = spawnTok.filename.empty() ? g_current_filename : spawnTok.filename;
    ...
    StmtPtr parse_stmt() {
        ...
        if (tok.kind == TokenKind::TK_IDENT) {
            ...
            if (tok.text == "if") {
                ...
                ifs->line = tok.line;
                ifs->col = tok.col;
                ifs->filename = tok.filename.empty() ? g_current_filename : tok.filename;
                return ifs;
            }
            ...
            if (tok.text == "for") {
                if (looks_like_foreach_loop()) {
                    ...
                    StmtPtr body = parse_block();
                    auto fs = std::make_unique<nari::ForEachStmt>(varTok.text, std::move(iterable), std::move(body));
                    fs->line = kw.line;
                    fs->col = kw.col;
                ...
                if (peek().kind != TokenKind::TK_SEMICOLON) {
                    if (peek().kind == TokenKind::TK_IDENT && (peek().text == "let" || peek().text == "global")) {
                        ...
                        if (peek().kind == TokenKind::TK_IDENT && peek(1).kind == TokenKind::TK_EQUAL) {
                            ...
                            assign->filename = nameTok.filename.empty() ? g_current_filename : nameTok.filename;
                ...
                if (peek().kind != TokenKind::TK_RPAREN) {
                    ...
                    // detect an assignment (=) and construct an AssignStmt for it.
                    // otherwise, parse an expression and wrap it in an ExprStmt as before.
                    if (peek().kind == TokenKind::TK_IDENT && peek(1).kind == TokenKind::TK_EQUAL) {
                        const Token &nameTok = peek();
                        std::string pname = next().text; // consume identifier
                        next(); // consume '='
            ...
            if (tok.text == "import") {
                ...
                if (peek().kind == TokenKind::TK_IDENT) {
                    ...
                    var_decl->filename = importTok.filename.empty() ? g_current_filename : importTok.filename;
            ...
            if (tok.text == "let" || tok.text == "global") {
                ...
                const Token &nameTok = peek();
                if (nameTok.kind != TokenKind::TK_IDENT) {
                    error_and_exit(std::string(is_global ? "Expected identifier after 'global'" : "Expected identifier after 'let'"));
                }
            ...
            if (next_kind == TokenKind::TK_PLUSEQ || next_kind == TokenKind::TK_MINUSEQ ||
                ...
                std::string binop;
                if (opTok.kind == TokenKind::TK_PLUSEQ) binop = "+";
                else if (opTok.kind == TokenKind::TK_MINUSEQ) binop = "-";
                else if (opTok.kind == TokenKind::TK_STAREQ) binop = "*";
                else if (opTok.kind == TokenKind::TK_SLASHEQ) binop = "/";
                else if (opTok.kind == TokenKind::TK_PERCENTEQ) binop = "%";
                else if (opTok.kind == TokenKind::TK_AMPEQ) binop = "&";
                else if (opTok.kind == TokenKind::TK_PIPEEQ) binop = "|";
                else if (opTok.kind == TokenKind::TK_CARETEQ) binop = "^";
                else if (opTok.kind == TokenKind::TK_LSHIFTEQ) binop = "<<";
                else if (opTok.kind == TokenKind::TK_RSHIFTEQ) binop = ">>";
                ...
                lhs->line = tok.line;
                lhs->col = tok.col;
                lhs->filename = tok.filename.empty() ? g_current_filename : tok.filename;

                auto binexpr = std::make_unique<nari::BinaryExpr>(binop, std::move(lhs), std::move(rhs));
                binexpr->line = opTok.line;
                binexpr->col = opTok.col;
                binexpr->filename = opTok.filename.empty() ? g_current_filename : opTok.filename;
            ...
            // check for indexed/member assignment: expr["member"] = val, or expr.member = val
            // this part is kinda a mess :)
                ...
                if (k == TokenKind::TK_LBRACKET || k == TokenKind::TK_DOT) {
                    ...
                    if (k == TokenKind::TK_LBRACKET) {
                        ...
                        while (lookahead_idx < toks.size() && depth > 0) {
                            if (peek(lookahead_idx).kind == TokenKind::TK_LBRACKET) depth++;
                            else if (peek(lookahead_idx).kind == TokenKind::TK_RBRACKET) depth--;
                            lookahead_idx++;
                        }
            ...
            if (is_indexed_assign) {
                // Parse the target as an expression (which will be IndexExpr or MemberExpr)
                ...
                idxAssignStmt->filename = tok.filename.empty() ? g_current_filename : tok.filename;
            ...
            if (peek(1).kind == TokenKind::TK_LPAREN) {
                ...
                exprStmt->filename = tok.filename.empty() ? g_current_filename : tok.filename;
                return exprStmt;
            }
        ...
        while (!is_eof() && peek().kind != TokenKind::TK_RBRACE) {
            ...
            if (peek().text == "case") {
                ...
                while (!is_eof() && peek().kind != TokenKind::TK_RBRACE) {
                    ...
                    if (st) body->stmts.push_back(std::move(st));
                    else break;
                }
            ...
            if (peek().text == "default") {
                if (seen_default) error_and_exit("Only one default case is allowed");
                seen_default = true;
    ...
    static Precedence token_precedence(const Token &t) {
        switch (t.kind) {
            case TokenKind::TK_QUESTION: return PREC_TERNARY;
            case TokenKind::TK_NULLISHCOALESCE: return PREC_NULL_COALESCE;
            case TokenKind::TK_OROR: return PREC_LOGICAL_OR;
            case TokenKind::TK_ANDAND: return PREC_LOGICAL_AND;
            case TokenKind::TK_PIPE: return PREC_BITWISE_OR;
            case TokenKind::TK_CARET: return PREC_BITWISE_XOR;
            case TokenKind::TK_AMPERSAND: return PREC_BITWISE_AND;
            case TokenKind::TK_EQEQ: case TokenKind::TK_NEQ: return PREC_EQUALITY;
            case TokenKind::TK_LT: case TokenKind::TK_GT: case TokenKind::TK_LE: case TokenKind::TK_GE: return PREC_COMPARISON;
            case TokenKind::TK_LSHIFT: case TokenKind::TK_RSHIFT: return PREC_SHIFT;
            case TokenKind::TK_PLUS: case TokenKind::TK_MINUS: case TokenKind::TK_AT: return PREC_TERM;
            case TokenKind::TK_STAR: case TokenKind::TK_SLASH: case TokenKind::TK_PERCENT: return PREC_FACTOR;
            case TokenKind::TK_EXPONENT: return PREC_EXPONENT;
    ...
    ExprPtr parse_precedence(Precedence min_prec) {
        ...
        while (true) {
            ...
            if (t.kind == TokenKind::TK_QUESTION) {
                ...
                auto te = std::make_unique<nari::TernaryExpr>(std::move(lhs), std::move(true_expr), std::move(false_expr));
                te->line = qmark.line;
                te->col = qmark.col;
                te->filename = qmark.filename.empty() ? g_current_filename : qmark.filename;
                lhs = std::move(te);
                continue;
            }
            ...
            std::string opname;
            switch (op.kind) {
                case TokenKind::TK_PLUS: opname = "+"; break;
                case TokenKind::TK_MINUS: opname = "-"; break;
                case TokenKind::TK_STAR: opname = "*"; break;
                case TokenKind::TK_AT: opname = "@"; break;
                case TokenKind::TK_SLASH: opname = "/"; break;
                case TokenKind::TK_PERCENT: opname = "%"; break;
                case TokenKind::TK_EXPONENT: opname = "**"; break;
                case TokenKind::TK_EQEQ: opname = "=="; break;
                case TokenKind::TK_NEQ: opname = "!="; break;
                case TokenKind::TK_LT: opname = "<"; break;
                case TokenKind::TK_GT: opname = ">"; break;
                case TokenKind::TK_LE: opname = "<="; break;
                case TokenKind::TK_GE: opname = ">="; break;
                case TokenKind::TK_ANDAND: opname = "&&"; break;
                case TokenKind::TK_OROR: opname = "||"; break;
                case TokenKind::TK_NULLISHCOALESCE: opname = "??"; break;
                case TokenKind::TK_AMPERSAND: opname = "&"; break;
                case TokenKind::TK_PIPE: opname = "|"; break;
                case TokenKind::TK_CARET: opname = "^"; break;
                case TokenKind::TK_LSHIFT: opname = "<<"; break;
                case TokenKind::TK_RSHIFT: opname = ">>"; break;
                default: opname = token_desc(op); break;
            }
    ...
    ExprPtr parse_unary() {
        ...
        if (t.kind == TokenKind::TK_BANG || t.kind == TokenKind::TK_MINUS || t.kind == TokenKind::TK_PLUS || t.kind == TokenKind::TK_TILDE) {
            ...
            if (!operand) error_and_exit("Expected operand for unary operator");
            std::string opname;
            if (op.kind == TokenKind::TK_BANG) opname = "!";
            else if (op.kind == TokenKind::TK_MINUS) opname = "neg";
            else if (op.kind == TokenKind::TK_TILDE) opname = "~";
            else opname = "+";
        ...
        if (t.kind == TokenKind::TK_PLUSPLUS || t.kind == TokenKind::TK_MINUSMINUS) {
            ...
            next();
            ExprPtr operand = parse_unary();
            if (!operand) error_and_exit("Expected operand for ++ or --");
            ...
            if (tok.kind == TokenKind::TK_PLUSPLUS || tok.kind == TokenKind::TK_MINUSMINUS) {
                ...
                if (peek().kind != TokenKind::TK_RPAREN) {
                    while (true) {
                        ...
                        if (peek().kind == TokenKind::TK_COMMA) { next(); continue; }
                        break;
                    }
                }
    ...
    nari::PatternPtr parse_pattern() {
        ...
        if (tok.kind == TokenKind::TK_IDENT && tok.text == "_") {
            ...
            pattern->col = tok.col;
            pattern->filename = tok.filename.empty() ? g_current_filename : tok.filename;
            return pattern;
        }
    ...
    ExprPtr parse_match_expression() {
        ...
        match_expr->filename = matchTok.filename.empty() ? g_current_filename : matchTok.filename;
        ...
        while (!is_eof() && peek().kind != TokenKind::TK_RBRACE) {
            ...
            // expect '=>'
            if (peek().kind == TokenKind::TK_EQUAL && peek(1).kind == TokenKind::TK_GT) {
                next(); // consume '='
                next(); // consume '>'
            } else {
    ...
    ExprPtr parse_primary() {
        ...
        if (tok.kind == TokenKind::TK_IDENT) {
            ...
            if (tok.text == "new") {
                ...
                new_expr->filename = tok.filename.empty() ? g_current_filename : tok.filename;
            ...
            str_expr->filename = tok.filename.empty() ? g_current_filename : tok.filename;
            next();
            return str_expr;
            ...
            interp->col = tok.col;
            interp->filename = tok.filename.empty() ? g_current_filename : tok.filename;
            ...
            if (is_float) {
                ...
                if (endptr == s.c_str() || errno == ERANGE) {
                    if (is_hex) {
                        ...
                        } else if (uv <= static_cast<unsigned long long>(std::numeric_limits<int64_t>::max())) {
                            num_expr = std::make_unique<nari::NumberExpr>(static_cast<int64_t>(uv));
                        } else {
                            num_expr = std::make_unique<nari::NumberExpr>(static_cast<double>(uv));
                        }
                    } else {
                        double dv = std::strtod(s.c_str(), &endptr);
                        if (endptr == s.c_str()) dv = 0.0;
            ...
            num_expr->filename = tok.filename.empty() ? g_current_filename : tok.filename;
            next();
            return num_expr;
        } else if (tok.kind == TokenKind::TK_LPAREN) {
            next(); // consume '('
            ...
            arr->line = start.line;
            arr->col = start.col;
            arr->filename = start.filename.empty() ? g_current_filename : start.filename;
            ...
            if (peek().kind != TokenKind::TK_RBRACKET) {
                while (true) {
                    ...
                    if (peek().kind == TokenKind::TK_COMMA) {
                        ...
                        if (peek().kind == TokenKind::TK_RBRACKET) break;
                        continue;
                    }
                    break;
                }
            ...
            obj->line = start.line;
            obj->col = start.col;
            obj->filename = start.filename.empty() ? g_current_filename : start.filename;
            ...
            if (peek().kind != TokenKind::TK_RBRACE) {
                while (true) {
                    ...
                    if (peek().kind == TokenKind::TK_COMMA) {
                        ...
                        if (peek().kind == TokenKind::TK_RBRACE) break;
                        continue;
                    }
                    break;
                }
...
std::vector<FunctionPtr> parse_program_from_source(const std::string &src, bool create_aggregator) {
    ...
    while (cur()) {
        ...
        if (c == '&' && peek() == '&') { push_tok(TokenKind::TK_ANDAND, "&&", line, col); advance(2); continue; }
        if (c == '&' && peek() == '=') { push_tok(TokenKind::TK_AMPEQ, "&=", line, col); advance(2); continue; }
        if (c == '|' && peek() == '|') { push_tok(TokenKind::TK_OROR, "||", line, col); advance(2); continue; }
        if (c == '|' && peek() == '=') { push_tok(TokenKind::TK_PIPEEQ, "|=", line, col); advance(2); continue; }
        if (c == '^' && peek() == '=') { push_tok(TokenKind::TK_CARETEQ, "^=", line, col); advance(2); continue; }
        if (c == '=' && peek() == '=') { push_tok(TokenKind::TK_EQEQ, "==", line, col); advance(2); continue; }
        if (c == '!' && peek() == '=') { push_tok(TokenKind::TK_NEQ, "!=", line, col); advance(2); continue; }
        if (c == '<' && peek() == '<' && peek(2) == '=') { push_tok(TokenKind::TK_LSHIFTEQ, "<<=", line, col); advance(3); continue; }
        if (c == '<' && peek() == '<') { push_tok(TokenKind::TK_LSHIFT, "<<", line, col); advance(2); continue; }
        if (c == '<' && peek() == '=') { push_tok(TokenKind::TK_LE, "<=", line, col); advance(2); continue; }
        if (c == '>' && peek() == '>' && peek(2) == '=') { push_tok(TokenKind::TK_RSHIFTEQ, ">>=", line, col); advance(3); continue; }
        if (c == '>' && peek() == '>') { push_tok(TokenKind::TK_RSHIFT, ">>", line, col); advance(2); continue; }
        if (c == '>' && peek() == '=') { push_tok(TokenKind::TK_GE, ">=", line, col); advance(2); continue; }
        if (c == '+' && peek() == '+') { push_tok(TokenKind::TK_PLUSPLUS, "++", line, col); advance(2); continue; }
        if (c == '-' && peek() == '-') { push_tok(TokenKind::TK_MINUSMINUS, "--", line, col); advance(2); continue; }
        if (c == '-' && peek() == '>') { push_tok(TokenKind::TK_ARROW, "->", line, col); advance(2); continue; }
        if (c == '+' && peek() == '=') { push_tok(TokenKind::TK_PLUSEQ, "+=", line, col); advance(2); continue; }
        if (c == '-' && peek() == '=') { push_tok(TokenKind::TK_MINUSEQ, "-=", line, col); advance(2); continue; }
        if (c == '*' && peek() == '*') { push_tok(TokenKind::TK_EXPONENT, "**", line, col); advance(2); continue; }
        if (c == '*' && peek() == '=') { push_tok(TokenKind::TK_STAREQ, "*=", line, col); advance(2); continue; }
        if (c == '/' && peek() == '=') { push_tok(TokenKind::TK_SLASHEQ, "/=", line, col); advance(2); continue; }
        if (c == '%' && peek() == '=') { push_tok(TokenKind::TK_PERCENTEQ, "%=", line, col); advance(2); continue; }
        if (c == '?' && peek() == '?') { push_tok(TokenKind::TK_NULLISHCOALESCE, "??", line, col); advance(2); continue; }
        if (c == '.' && peek() == '.' && peek(2) == '.') { push_tok(TokenKind::TK_ELLIPSIS, "...", line, col); advance(3); continue; }

        // single-char tokens
        if (c == '(') { push_tok(TokenKind::TK_LPAREN, "(", line, col); advance(); continue; }
        if (c == ')') { push_tok(TokenKind::TK_RPAREN, ")", line, col); advance(); continue; }
        if (c == '{') { push_tok(TokenKind::TK_LBRACE, "{", line, col); advance(); continue; }
        if (c == '}') { push_tok(TokenKind::TK_RBRACE, "}", line, col); advance(); continue; }
        if (c == '[') { push_tok(TokenKind::TK_LBRACKET, "[", line, col); advance(); continue; }
        if (c == ']') { push_tok(TokenKind::TK_RBRACKET, "]", line, col); advance(); continue; }
        if (c == ',') { push_tok(TokenKind::TK_COMMA, ",", line, col); advance(); continue; }
        if (c == ';') { push_tok(TokenKind::TK_SEMICOLON, ";", line, col); advance(); continue; }
        if (c == ':') { push_tok(TokenKind::TK_COLON, ":", line, col); advance(); continue; }
        if (c == '.') { push_tok(TokenKind::TK_DOT, ".", line, col); advance(); continue; }
        if (c == '?') { push_tok(TokenKind::TK_QUESTION, "?", line, col); advance(); continue; }
        if (c == '@') { push_tok(TokenKind::TK_AT, "@", line, col); advance(); continue; }
        if (c == '=') { push_tok(TokenKind::TK_EQUAL, "=", line, col); advance(); continue; }
        if (c == '+') { push_tok(TokenKind::TK_PLUS, "+", line, col); advance(); continue; }
        if (c == '-') { push_tok(TokenKind::TK_MINUS, "-", line, col); advance(); continue; }
        if (c == '*') { push_tok(TokenKind::TK_STAR, "*", line, col); advance(); continue; }
        if (c == '/') { push_tok(TokenKind::TK_SLASH, "/", line, col); advance(); continue; }
        if (c == '!') { push_tok(TokenKind::TK_BANG, "!", line, col); advance(); continue; }
        if (c == '<') { push_tok(TokenKind::TK_LT, "<", line, col); advance(); continue; }
        if (c == '>') { push_tok(TokenKind::TK_GT, ">", line, col); advance(); continue; }
        if (c == '%') { push_tok(TokenKind::TK_PERCENT, "%", line, col); advance(); continue; }
        if (c == '&') { push_tok(TokenKind::TK_AMPERSAND, "&", line, col); advance(); continue; }
        if (c == '|') { push_tok(TokenKind::TK_PIPE, "|", line, col); advance(); continue; }
        if (c == '^') { push_tok(TokenKind::TK_CARET, "^", line, col); advance(); continue; }
        if (c == '~') { push_tok(TokenKind::TK_TILDE, "~", line, col); advance(); continue; }
        ...
        if (c == '`') {
            ...
            while (cur() && cur() != '`') {
                if (cur() == '\\') {
                    ...
                    char e = cur();
                    if (!e) break;
                    if (e == 'n') current_part.push_back('\n');
                    else if (e == 'r') current_part.push_back('\r');
                    else if (e == 't') current_part.push_back('\t');
                    else if (e == '\\') current_part.push_back('\\');
                    else if (e == '`') current_part.push_back('`');
                    else if (e == '{') current_part.push_back('{');
                    else current_part.push_back(e);
                    advance();
                } else if (cur() == '{') {
        ...
        if (isalpha(c) || c == '_') {
            ...
            size_t st = pos;
            advance();
            while (cur() && (isalnum(cur()) || cur() == '_')) advance();
