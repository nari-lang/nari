...
struct ASTNode {
    ...
    std::string loc_str() const {
        if (filename.empty() && line == 0 && col == 0) return "";
        if (!filename.empty()) {
...
enum class ExprKind {
    Ident, String, Number, Bool, Null,
    Unary, Binary, Call, Ternary, Match,
    Spawn, StringInterpolation, ArrayLiteral, ObjectLiteral,
    Function, Index, Member, This, New, Other
};
...
struct IdentExpr : Expr {
    ...
    explicit IdentExpr(std::string n) : name(std::move(n)) { kind = ExprKind::Ident; }
...
struct FunctionExpr : Expr {
    ...
    FunctionExpr() { kind = ExprKind::Function; }
...
struct StringExpr : Expr {
    ...
    explicit StringExpr(std::string v) : value(std::move(v)) { kind = ExprKind::String; }
...
struct NumberExpr : Expr {
    ...
    explicit NumberExpr(double v) : is_float(true), f(v) { kind = ExprKind::Number; }
    explicit NumberExpr(int64_t v) : is_float(false), i(v) { kind = ExprKind::Number; }
...
struct BoolExpr : Expr {
    ...
    explicit BoolExpr(bool v) : value(v) { kind = ExprKind::Bool; }
...
struct UnaryExpr : Expr {
    ...
    UnaryExpr(std::string o, ExprPtr opd) : op(std::move(o)), operand(std::move(opd)) { kind = ExprKind::Unary; }

    void pretty_print(int indent = 0) const override {
        print_indent(indent);
        printf("UnaryExpr: %s%s\n", op.c_str(), loc_str().c_str());
        if (operand) operand->pretty_print(indent + 2);
    }
};
...
struct BinaryExpr : Expr {
    ...
    BinaryExpr(std::string o) : op(std::move(o)) { kind = ExprKind::Binary; }
    BinaryExpr(std::string o, ExprPtr l, ExprPtr r) : op(std::move(o)), left(std::move(l)), right(std::move(r)) { kind = ExprKind::Binary; }

    void pretty_print(int indent = 0) const override {
        print_indent(indent);
        printf("BinaryExpr: %s%s\n", op.c_str(), loc_str().c_str());
        if (left) left->pretty_print(indent + 2);
        if (right) right->pretty_print(indent + 2);
    }
};
...
struct CallExpr : Expr {
    ...
    explicit CallExpr(ExprPtr c) : callee(std::move(c)) { kind = ExprKind::Call; }
    void pretty_print(int indent = 0) const override {
        print_indent(indent);
        printf("CallExpr:\n");
        ...
        print_indent(indent);
        printf("Args:\n");
        for (const auto &a : args) a->pretty_print(indent + 2);
    }
};
...
struct ArrayLiteralExpr : Expr {
    ...
    ArrayLiteralExpr() { kind = ExprKind::ArrayLiteral; }
    void pretty_print(int indent = 0) const override {
        print_indent(indent);
        printf("ArrayLiteral%s [\n", loc_str().c_str());
        for (const auto &elem : elements) {
            if (elem) elem->pretty_print(indent + 2);
        }
        print_indent(indent);
        printf("]\n");
    }
...
struct ObjectLiteralExpr : Expr {
    ...
    ObjectLiteralExpr() { kind = ExprKind::ObjectLiteral; }
    ...
    void pretty_print(int indent = 0) const override {
        ...
        for (const auto &[key, val] : entries) {
            print_indent(indent + 2);
            printf("%s:\n", key.c_str());
            if (val) val->pretty_print(indent + 4);
        }
        print_indent(indent);
        printf("}\n");
    }
...
struct IndexExpr : Expr {
    ExprPtr object;  // array or object
    ExprPtr index;   // index or key

    IndexExpr(ExprPtr obj, ExprPtr idx) : object(std::move(obj)), index(std::move(idx)) { kind = ExprKind::Index; }
    ...
    void pretty_print(int indent = 0) const override {
        ...
        print_indent(indent + 2);
        printf("Object:\n");
        if (object) object->pretty_print(indent + 4);
        print_indent(indent + 2);
        printf("Index:\n");
        if (index) index->pretty_print(indent + 4);
    }
};

// member access: object.member
struct MemberExpr : Expr {
    ExprPtr object;      // object being accessed
    std::string member;  // member name
    MemberExpr(ExprPtr obj, std::string mem) : object(std::move(obj)), member(std::move(mem)) { kind = ExprKind::Member; }
    void pretty_print(int indent = 0) const override {
        print_indent(indent);
        printf("MemberExpr%s .%s:\n", loc_str().c_str(), member.c_str());
        if (object) object->pretty_print(indent + 2);
    }
};
...
struct NewExpr : Expr {
    ...
    explicit NewExpr(std::string cn) : class_name(std::move(cn)) { kind = ExprKind::New; }
...
struct StringInterpolationExpr : Expr {
    ...
    std::vector<std::string> expr_sources;    // expression to parse

    StringInterpolationExpr() { kind = ExprKind::StringInterpolation; }
...
struct TernaryExpr : Expr {
    ...
    TernaryExpr(ExprPtr cond, ExprPtr true_val, ExprPtr false_val)
        : condition(std::move(cond)), true_expr(std::move(true_val)), false_expr(std::move(false_val)) { kind = ExprKind::Ternary; }
    ...
    void pretty_print(int indent = 0) const override {
        ...
        print_indent(indent + 2);
        printf("Condition:\n");
        if (condition) condition->pretty_print(indent + 4);
        print_indent(indent + 2);
        printf("True:\n");
        if (true_expr) true_expr->pretty_print(indent + 4);
        print_indent(indent + 2);
        printf("False:\n");
        if (false_expr) false_expr->pretty_print(indent + 4);
    }
};

// type tags for fast stmt dispatch
enum class StmtKind {
    Expr, VarDecl, Assign, IndexAssign, Block,
    If, While, For, ForEach, Switch,
    Break, Continue, Return, Throw, Try, Other
};
...
struct ExprStmt : Stmt {
    ...
    explicit ExprStmt(ExprPtr e) : expr(std::move(e)) { stmt_kind = StmtKind::Expr; }

    void pretty_print(int indent = 0) const override {
        print_indent(indent);
        printf("ExprStmt%s:\n", loc_str().c_str());
        if (expr) expr->pretty_print(indent + 2);
    }
};
...
struct VarDeclStmt : Stmt {
    ...
    VarDeclStmt(std::string n, ExprPtr i, VarDeclCtrl g = LOCAL) : name(std::move(n)), initializerExpr(std::move(i)), is_global(g) { stmt_kind = StmtKind::VarDecl; }

    void pretty_print(int indent = 0) const override {
        print_indent(indent);
        if (is_global) printf("VarDecl (global): %s%s =\n", name.c_str(), loc_str().c_str());
        else printf("VarDecl: %s%s =\n", name.c_str(), loc_str().c_str());
        if (initializerExpr) initializerExpr->pretty_print(indent + 2);
    }
};
...
struct AssignStmt : Stmt {
    ...
    AssignStmt(std::string t, ExprPtr v) : target(std::move(t)), value(std::move(v)) { stmt_kind = StmtKind::Assign; }

    void pretty_print(int indent = 0) const override {
        print_indent(indent);
        printf("Assign: %s%s =\n", target.c_str(), loc_str().c_str());
        if (value) value->pretty_print(indent + 2);
    }
};
...
struct IndexAssignStmt : Stmt {
    ...
    IndexAssignStmt(ExprPtr t, ExprPtr v) : target(std::move(t)), value(std::move(v)) { stmt_kind = StmtKind::IndexAssign; }
    ...
    void pretty_print(int indent = 0) const override {
        ...
        print_indent(indent + 2);
        printf("Target:\n");
        if (target) target->pretty_print(indent + 4);
        print_indent(indent + 2);
        printf("Value:\n");
        if (value) value->pretty_print(indent + 4);
    }
};
...
struct IfStmt : Stmt {
    ...
    IfStmt(ExprPtr c, StmtPtr t, StmtPtr e = nullptr) : cond(std::move(c)), then_branch(std::move(t)), else_branch(std::move(e)) { stmt_kind = StmtKind::If; }
...
struct WhileStmt : Stmt {
    ...
    WhileStmt(ExprPtr c, StmtPtr b) : cond(std::move(c)), body(std::move(b)) { stmt_kind = StmtKind::While; }
...
struct ForStmt : Stmt {
    ...
    ForStmt(StmtPtr i, ExprPtr c, StmtPtr p, StmtPtr b) : init(std::move(i)), cond(std::move(c)), post(std::move(p)), body(std::move(b)) { stmt_kind = StmtKind::For; }
...
struct ForEachStmt : Stmt {
    ...
    ForEachStmt(std::string v, ExprPtr it, StmtPtr b)
        : var(std::move(v)), iterable(std::move(it)), body(std::move(b)) { stmt_kind = StmtKind::ForEach; }
...
struct BreakStmt : Stmt {
    BreakStmt() { stmt_kind = StmtKind::Break; }
    void pretty_print(int indent = 0) const override {
        print_indent(indent);
        printf("Break%s\n", loc_str().c_str());
    }
...
struct ContinueStmt : Stmt {
    ContinueStmt() { stmt_kind = StmtKind::Continue; }
    void pretty_print(int indent = 0) const override {
        print_indent(indent);
        printf("Continue%s\n", loc_str().c_str());
    }
...
struct ReturnStmt : Stmt {
    ...
    explicit ReturnStmt(ExprPtr v = nullptr) : value(std::move(v)) { stmt_kind = StmtKind::Return; }
...
struct ThrowStmt : Stmt {
    ...
    explicit ThrowStmt(ExprPtr v) : value(std::move(v)) { stmt_kind = StmtKind::Throw; }
...
struct BlockStmt : Stmt {
    ...
    BlockStmt() { stmt_kind = StmtKind::Block; }

    void pretty_print(int indent = 0) const override {
        print_indent(indent);
        printf("Block%s\n", loc_str().c_str());
        for (const auto &s : stmts) {
            if (s) s->pretty_print(indent + 2);
        }
    }
};
...
struct SpawnExpr : Expr {
    ...
    SpawnExpr(BlockPtr b) : body(std::move(b)) { kind = ExprKind::Spawn; }
...
struct SwitchStmt : Stmt {
    ...
    explicit SwitchStmt(ExprPtr v) : value(std::move(v)) { stmt_kind = StmtKind::Switch; }
    ...
    void pretty_print(int indent = 0) const override {
        ...
        for (const auto &c : cases) {
            print_indent(indent + 2);
            printf("Case:\n");
            if (c.match) c.match->pretty_print(indent + 4);
            if (c.body) c.body->pretty_print(indent + 4);
        }
        if (default_body) {
            print_indent(indent + 2);
            printf("Default:\n");
...
struct TryStmt : Stmt {
    ...
    TryStmt(BlockPtr t, std::string cv, BlockPtr c, BlockPtr f)
        : try_block(std::move(t)), catch_var(std::move(cv)), catch_block(std::move(c)), finally_block(std::move(f)) { stmt_kind = StmtKind::Try; }
...
struct TypeDecl : ASTNode {
    ...
    bool is_alias() const { return alias_target != nullptr; }
...
struct EnumVariant {
    ...
    bool is_unit() const { return tuple_fields.empty() && named_fields.empty(); }
    bool is_tuple() const { return !tuple_fields.empty(); }
    bool is_struct() const { return !named_fields.empty(); }
};
...
struct ClassMethod {
    ...
    ClassMethod(std::string n, Visibility v)
...
enum class PatternKind {
    Wildcard, Binding, Literal, Variant, Other
};
...
struct WildcardPattern : Pattern {
    WildcardPattern() { pattern_kind = PatternKind::Wildcard; }
    void pretty_print(int indent = 0) const override {
        print_indent(indent);
        printf("_\n");
    }
...
struct BindingPattern : Pattern {
    ...
    explicit BindingPattern(std::string n) : name(std::move(n)) { pattern_kind = PatternKind::Binding; }
...
struct LiteralPattern : Pattern {
    ...
    explicit LiteralPattern(ExprPtr v) : value(std::move(v)) { pattern_kind = PatternKind::Literal; }
...
struct VariantPattern : Pattern {
    std::string enum_name;     // Optional: Result, Option, etc.
    std::string variant_name;  // Ok, Err, Some, None
    std::vector<PatternPtr> fields;  // Nested patterns for variant data

    explicit VariantPattern(std::string v) : variant_name(std::move(v)) { pattern_kind = PatternKind::Variant; }
    VariantPattern(std::string e, std::string v) : enum_name(std::move(e)), variant_name(std::move(v)) { pattern_kind = PatternKind::Variant; }
    ...
    void pretty_print(int indent = 0) const override {
        ...
        if (!fields.empty()) {
            printf("(\n");
            for (const auto &field : fields) {
                if (field) field->pretty_print(indent + 2);
            }
            print_indent(indent);
            printf(")");
        }
...
struct MatchExpr : Expr {
    ...
    explicit MatchExpr(ExprPtr s) : scrutinee(std::move(s)) { kind = ExprKind::Match; }
    ...
    void pretty_print(int indent = 0) const override {
        ...
        print_indent(indent + 2);
        printf("Scrutinee:\n");
        if (scrutinee) scrutinee->pretty_print(indent + 4);
        print_indent(indent + 2);
        printf("Arms:\n");
        for (const auto &arm : arms) {
            print_indent(indent + 4);
            printf("Pattern:\n");
            if (arm.pattern) arm.pattern->pretty_print(indent + 6);
            print_indent(indent + 4);
            printf("Body:\n");
            if (arm.body) arm.body->pretty_print(indent + 6);
        }
    }
};
...
struct Function : ASTNode {
    ...
    ~Function() {
        // Clean up closure environment using custom deleter
    ...
    void pretty_print(int indent) const {
        ...
        if (body) body->pretty_print(indent + 2);
    }
};
...
struct Program {
    ...
    void pretty_print(int indent = 0) const {
        ...
        for (const auto &t : types) {
            if (t) t->pretty_print(indent + 2);
        }
        for (const auto &e : enums) {
            if (e) e->pretty_print(indent + 2);
        }
        for (const auto &c : classes) {
            if (c) c->pretty_print(indent + 2);
        }
        for (const auto &f : functions) {
            if (f) f->pretty_print(indent + 2);
        }
        if (!top_level_stmts.empty()) {
            print_indent(indent + 2);
            printf("Top-level statements:\n");
            for (const auto &s : top_level_stmts) {
                if (s) s->pretty_print(indent + 4);
            }
        }
    }
};
