...
struct TypeAnnotation {
  ...
  explicit TypeAnnotation(std::string n, bool arr = false)
      : name(std::move(n)), is_array(arr) {}
...
struct Param {
  ...
  Param(std::string n, ExprPtr def = nullptr, bool rest = false,
        TypeAnnotationPtr t = nullptr)
      : name(std::move(n)), default_value(std::move(def)), is_rest(rest),
        type(std::move(t)) {}
};
...
struct UnaryExpr : Expr {
  ...
  UnaryExpr(std::string o, ExprPtr opd)
...
struct TernaryExpr : Expr {
  ...
  TernaryExpr(ExprPtr cond, ExprPtr true_val, ExprPtr false_val)
      : condition(std::move(cond)), true_expr(std::move(true_val)),
...
// indexed assignment: arr[index] = value, obj[key] = value, or obj.member =
// value
struct IndexAssignStmt : Stmt {
  ExprPtr target; // should be IndexExpr or MemberExpr
  ExprPtr value;

  IndexAssignStmt(ExprPtr t, ExprPtr v)
      : target(std::move(t)), value(std::move(v)) {
...
struct ForStmt : Stmt {
  ...
  ForStmt(StmtPtr i, ExprPtr c, StmtPtr p, StmtPtr b)
      : init(std::move(i)), cond(std::move(c)), post(std::move(p)),
        body(std::move(b)) {
...
struct TryStmt : Stmt {
  ...
  TryStmt(BlockPtr t, std::string cv, BlockPtr c, BlockPtr f)
      : try_block(std::move(t)), catch_var(std::move(cv)),
        catch_block(std::move(c)), finally_block(std::move(f)) {
...
struct TypeField {
  ...
  TypeField(std::string n, TypeAnnotationPtr t)
      : name(std::move(n)), type(std::move(t)) {}
};

// custom type declaration: type Name<T, U> { field: type; ... }, and type
// aliases (type NameAlias Name)
...
struct ClassField {
  ...
  ClassField(std::string n, Visibility v, TypeAnnotationPtr t,
             ExprPtr def = nullptr)
      : name(std::move(n)), visibility(v), type(std::move(t)),
...
struct VariantPattern : Pattern {
  ...
  VariantPattern(std::string e, std::string v)
      : enum_name(std::move(e)), variant_name(std::move(v)) {
    pattern_kind = PatternKind::Variant;
  }
...
struct MatchArm {
  ...
  MatchArm(PatternPtr p, ExprPtr b)
...
struct Function : ASTNode {
  ...
  const FunctionExpr *function_expr =
      nullptr; // for lambda functions created from FunctionExpr
