...
std::unordered_map<FFISignature, FFICaller::CIFCache, FFISignatureHash> FFICaller::cif_cache_;
std::unordered_map<FFISignature, FFICaller::CIFCache, FFISignatureHash> FFICaller::cif_variadic_cache_;
...
static ffi_type* get_ffi_type(FFIType type) {
    switch (type) {
        ...
        case FFIType::Bool: return &ffi_type_uchar;  // C99 bool is unsigned char sized
        case FFIType::Pointer: return &ffi_type_pointer;
        case FFIType::Struct: return nullptr; // structs need custom handling
        default: return &ffi_type_void;
    }
}

// create ffi_type for a struct definition
static ffi_type* get_ffi_struct_type(FFIStructDef* struct_def) {
    if (!struct_def) return &ffi_type_void;
...
FFICaller::CIFCache& FFICaller::get_or_create_cif(const FFISignature& sig) {
    ...
    if (status != FFI_OK) {
        // return a default-constructed cache on error
        static CIFCache error_cache;
        return error_cache;
    }

    return cif_cache_.emplace(sig, std::move(cache)).first->second;
}

// get or create cached CIF for variadic functions
FFICaller::CIFCache& FFICaller::get_or_create_cif_variadic(const FFISignature& sig) {
    auto it = cif_variadic_cache_.find(sig);
    if (it != cif_variadic_cache_.end()) {
        return it->second;
    }
    ...
    ffi_status status = ffi_prep_cif_var(&cache.cif, FFI_DEFAULT_ABI, sig.fixed_param_count, cache.param_types.size(), return_type, cache.param_types.data());
    ...
    return cif_variadic_cache_.emplace(sig, std::move(cache)).first->second;
}
...
Value FFICaller::call_function(void* func_ptr, const FFISignature& sig, const std::vector<Value>& args) {
    ...
    // reserve space because vector relocation yayy
    std::vector<std::string> string_storage;
    string_storage.reserve(args.size());

    // todo: this is a bit gross but we need to store the converted argument values somewhere so we can pass pointers to them to ffi_call. 
    // we can't just convert and pass on the fly because the storage needs to persist until after ffi_call returns. 
    // we also need to handle string storage separately since we need to keep the std::string objects alive while passing their c_str() pointers to ffi.
    ...
    std::vector<std::vector<char>> struct_storage; // buffers for struct data
    int32_storage.reserve(args.size());
    int64_storage.reserve(args.size());
    int8_storage.reserve(args.size());
    uint8_storage.reserve(args.size());
    int16_storage.reserve(args.size());
    uint16_storage.reserve(args.size());
    uint32_storage.reserve(args.size());
    uint64_storage.reserve(args.size());
    float_storage.reserve(args.size());
    double_storage.reserve(args.size());
    bool_storage.reserve(args.size());
    pointer_storage.reserve(args.size());
    struct_storage.reserve(args.size());
        ...
        switch (type) {
            case FFIType::Int8: {
                int8_t val = arg.is_int() ? static_cast<int8_t>(arg.get_int()) :
                             arg.is_float() ? static_cast<int8_t>(arg.get_float()) : 0;
            ...
            case FFIType::UInt8: {
                uint8_t val = arg.is_int() ? static_cast<uint8_t>(arg.get_int()) :
                              ...
                              arg.is_bool() ? (arg.get_bool() ? 1u : 0u) : 0u;
            ...
            case FFIType::Int16: {
                int16_t val = arg.is_int() ? static_cast<int16_t>(arg.get_int()) :
                              arg.is_float() ? static_cast<int16_t>(arg.get_float()) : 0;
            ...
            case FFIType::Int32: {
                // if int, get int, if float, get float
                int32_t val = arg.is_int() ? arg.get_int() :
                              arg.is_float() ? (int32_t)arg.get_float() : 0;
            ...
            case FFIType::Int64: {
                int64_t val = arg.is_int() ? arg.get_int() :
                              arg.is_float() ? (int64_t)arg.get_float() : 0;
            ...
            case FFIType::UInt64: {
                uint64_t val = arg.is_int() ? static_cast<uint64_t>(arg.get_int()) :
                               ...
                               arg.is_bool() ? (arg.get_bool() ? 1ull : 0ull) : 0ull;
            ...
            case FFIType::Float: {
                float val = arg.is_float() ? (float)arg.get_float() :
                            arg.is_int() ? (float)arg.get_int() : 0.0f;
            ...
            case FFIType::Double: {
                double val = arg.is_float() ? arg.get_float() :
                             arg.is_int() ? (double)arg.get_int() : 0.0;
            ...
            case FFIType::Bool: {
                uint8_t val = arg.is_bool() ? (arg.get_bool() ? 1 : 0) :
                              arg.is_int() ? (arg.get_int() != 0 ? 1 : 0) : 0;
            ...
            case FFIType::Struct: {
                ...
                auto obj = arg.get_object();
                
                // allocate buffer for struct and try to estimate size
                size_t struct_size = 0;
                for (const auto& field : struct_def->fields) {
                    switch (field.type) {
                        case FFIType::Int8: struct_size += sizeof(int8_t); break;
                        case FFIType::UInt8: struct_size += sizeof(uint8_t); break;
                        case FFIType::Int16: struct_size += sizeof(int16_t); break;
                        case FFIType::UInt16: struct_size += sizeof(uint16_t); break;
                        case FFIType::Int32: struct_size += sizeof(int32_t); break;
                        case FFIType::Int64: struct_size += sizeof(int64_t); break;
                        case FFIType::UInt32: struct_size += sizeof(uint32_t); break;
                        case FFIType::UInt64: struct_size += sizeof(uint64_t); break;
                        case FFIType::Float: struct_size += sizeof(float); break;
                        case FFIType::Double: struct_size += sizeof(double); break;
                        case FFIType::Bool: struct_size += sizeof(uint8_t); break;
                        case FFIType::Pointer: struct_size += sizeof(void*); break;
                        default: break;
                    }
                }
                ...
                for (const auto& field : struct_def->fields) {
                    ...
                    const Value& field_val = field_it->second;
                    
                    switch (field.type) {
                        case FFIType::Int8: {
                            int8_t val = field_val.is_int() ? static_cast<int8_t>(field_val.get_int()) :
                                         field_val.is_float() ? static_cast<int8_t>(field_val.get_float()) : 0;
                            memcpy(struct_buf + offset, &val, sizeof(int8_t));
                            offset += sizeof(int8_t);
                            break;
                        }
                        case FFIType::UInt8: {
                            uint8_t val = field_val.is_int() ? static_cast<uint8_t>(field_val.get_int()) :
                                          field_val.is_float() ? static_cast<uint8_t>(field_val.get_float()) :
                                          field_val.is_bool() ? (field_val.get_bool() ? 1u : 0u) : 0u;
                            memcpy(struct_buf + offset, &val, sizeof(uint8_t));
                            offset += sizeof(uint8_t);
                            break;
                        }
                        case FFIType::Int16: {
                            int16_t val = field_val.is_int() ? static_cast<int16_t>(field_val.get_int()) :
                                          field_val.is_float() ? static_cast<int16_t>(field_val.get_float()) : 0;
                            memcpy(struct_buf + offset, &val, sizeof(int16_t));
                            offset += sizeof(int16_t);
                            break;
                        }
                        case FFIType::UInt16: {
                            uint16_t val = field_val.is_int() ? static_cast<uint16_t>(field_val.get_int()) :
                        ...
                        case FFIType::Int32: {
                            int32_t val = field_val.is_int() ? (int32_t)field_val.get_int() :
                                          field_val.is_float() ? (int32_t)field_val.get_float() : 0;
                        ...
                        case FFIType::Int64: {
                            int64_t val = field_val.is_int() ? field_val.get_int() :
                                          field_val.is_float() ? (int64_t)field_val.get_float() : 0;
                            memcpy(struct_buf + offset, &val, sizeof(int64_t));
                            offset += sizeof(int64_t);
                            break;
                        }
                        case FFIType::UInt32: {
                            uint32_t val = field_val.is_int() ? static_cast<uint32_t>(field_val.get_int()) :
                        ...
                        case FFIType::UInt64: {
                            uint64_t val = field_val.is_int() ? static_cast<uint64_t>(field_val.get_int()) :
                                           field_val.is_float() ? static_cast<uint64_t>(field_val.get_float()) :
                                           field_val.is_bool() ? (field_val.get_bool() ? 1ull : 0ull) : 0ull;
                        ...
                        case FFIType::Float: {
                            float val = field_val.is_float() ? (float)field_val.get_float() :
                                        field_val.is_int() ? (float)field_val.get_int() : 0.0f;
                        ...
                        case FFIType::Double: {
                            double val = field_val.is_float() ? field_val.get_float() :
                                         field_val.is_int() ? (double)field_val.get_int() : 0.0;
                        ...
                        case FFIType::Bool: {
                            uint8_t val = field_val.is_bool() ? (field_val.get_bool() ? 1 : 0) :
                                          field_val.is_int() ? (field_val.get_int() != 0 ? 1 : 0) : 0;
                            memcpy(struct_buf + offset, &val, sizeof(uint8_t));
                            offset += sizeof(uint8_t);
                            break;
                        }
    ...
    switch (sig.return_type) {
        ...
        case FFIType::Struct: {
            ...
            auto result_obj = std::make_shared<std::unordered_map<std::string, Value>>();
            size_t offset = 0;
            ...
            for (const auto& field : sig.return_struct_def->fields) {
                switch (field.type) {
                    case FFIType::Int8: {
                        ...
                        (*result_obj)[field.name] = Value::make_int(val);
                        offset += sizeof(int8_t);
                        break;
                    }
                    case FFIType::UInt8: {
                        ...
                        (*result_obj)[field.name] = Value::make_int(static_cast<int64_t>(val));
                        offset += sizeof(uint8_t);
                        break;
                    }
                    case FFIType::Int16: {
                    ...
                    case FFIType::Float: {
                        ...
                        (*result_obj)[field.name] = Value::make_float(val);
                        offset += sizeof(float);
                        break;
                    }
                    case FFIType::Double: {
                    ...
                    case FFIType::Bool: {
                        uint8_t val;
                        memcpy(&val, return_value.struct_buf + offset, sizeof(uint8_t));
                        (*result_obj)[field.name] = Value::make_bool(val != 0);
                        offset += sizeof(uint8_t);
                        break;
                    }
                    case FFIType::Pointer: {
                        ...
                        (*result_obj)[field.name] = Value::make_int((int64_t)ptr);
                        offset += sizeof(void*);
                        break;
                    }
                    default:
            ...
            return Value::make_object(result_obj);
        }
    }
...
std::shared_ptr<FFIStructDef> create_struct_def_from_type(const std::string& type_name) {
    ...
    auto map_type_to_ffi = [](const std::string& nari_type) -> FFIType {
        if (nari_type == "f32" || nari_type == "float") return FFIType::Float;
        if (nari_type == "f64" || nari_type == "double") return FFIType::Double;
        if (nari_type == "i8") return FFIType::Int8;
        if (nari_type == "u8") return FFIType::UInt8;
        if (nari_type == "i16") return FFIType::Int16;
        if (nari_type == "u16") return FFIType::UInt16;
        if (nari_type == "i32" || nari_type == "int") return FFIType::Int32;
        if (nari_type == "i64" || nari_type == "long") return FFIType::Int64;
        if (nari_type == "u32" || nari_type == "uint") return FFIType::UInt32;
        if (nari_type == "u64" || nari_type == "ulong") return FFIType::UInt64;
        if (nari_type == "bool" || nari_type == "boolean") return FFIType::Bool;
        if (nari_type == "string" || nari_type == "pointer") return FFIType::Pointer;
    ...
    for (const auto& field : struct_def->fields) {
        switch (field.type) {
            ...
            case FFIType::Pointer: {
                ...
                (*result_obj)[field.name] = Value::make_int(reinterpret_cast<int64_t>(ptr_val));
                offset += sizeof(void*);
                break;
            }
            default:
...
void write_struct_to_memory(void* ptr, const std::string& type_name, const Value& obj) {
    ...
    auto obj_map = obj.get_object();
    char* struct_buf = (char*)ptr;
    size_t offset = 0;
    
    for (const auto& field : struct_def->fields) {
        auto field_it = obj_map->find(field.name);
        if (field_it == obj_map->end()) {
        ...
        switch (field.type) {
            ...
            case FFIType::Int32: {
                int32_t val = field_val.is_int() ? static_cast<int32_t>(field_val.get_int()) :
                              field_val.is_float() ? static_cast<int32_t>(field_val.get_float()) : 0;
                memcpy(struct_buf + offset, &val, sizeof(int32_t));
                offset += sizeof(int32_t);
                break;
            }
            case FFIType::Int64: {
                int64_t val = field_val.is_int() ? field_val.get_int() :
                              field_val.is_float() ? static_cast<int64_t>(field_val.get_float()) : 0;
                memcpy(struct_buf + offset, &val, sizeof(int64_t));
                offset += sizeof(int64_t);
                break;
            }
            ...
            case FFIType::Float: {
                float val = field_val.is_float() ? static_cast<float>(field_val.get_float()) :
                            field_val.is_int() ? static_cast<float>(field_val.get_int()) : 0.0f;
                memcpy(struct_buf + offset, &val, sizeof(float));
                offset += sizeof(float);
                break;
            }
            case FFIType::Double: {
                double val = field_val.is_float() ? field_val.get_float() :
                             field_val.is_int() ? static_cast<double>(field_val.get_int()) : 0.0;
                memcpy(struct_buf + offset, &val, sizeof(double));
                offset += sizeof(double);
                break;
            }
...
void FFICallbackManager::callback_trampoline(ffi_cif* cif, void* ret, void** args, void* user_data) {
    ...
    // call nari func
    Value result = callback->runtime->call_function_value(callback->nari_function, nari_args);
...
void* FFICallbackManager::create_callback(const FFISignature& sig, const Value& nari_func, ScriptRuntime* runtime) {
    ...
    callbacks_[code_ptr] = std::move(callback);
    
    return code_ptr;
}

void FFICallbackManager::free_callback(void* code_ptr) {
    auto it = callbacks_.find(code_ptr);
