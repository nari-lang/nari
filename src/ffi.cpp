...
std::unordered_map<FFISignature, FFICaller::CIFCache, FFISignatureHash>
    FFICaller::cif_cache_;
std::unordered_map<FFISignature, FFICaller::CIFCache, FFISignatureHash>
    FFICaller::cif_variadic_cache_;
...
void FFILibrary::enumerate_symbols() {
  ...
  const auto *name_rvas =
      reinterpret_cast<const DWORD *>(module_base + export_dir->AddressOfNames);
  ...
  for (DWORD i = 0; i < export_dir->NumberOfNames; ++i) {
    ...
    const char *symbol_name =
        reinterpret_cast<const char *>(module_base + name_rva);
...
static ffi_type *get_ffi_type(FFIType type) {
  ...
  case FFIType::Bool:
    return &ffi_type_uchar; // C99 bool is unsigned char sized
...
FFICaller::CIFCache &FFICaller::get_or_create_cif(const FFISignature &sig) {
  ...
  for (size_t i = 0; i < sig.param_types.size(); i++) {
    ...
    if (type == FFIType::Struct && i < sig.param_struct_defs.size()) {
      cache.param_types.push_back(
          get_ffi_struct_type(sig.param_struct_defs[i].get()));
    } else {
  ...
  ffi_status status =
      ffi_prep_cif(&cache.cif, FFI_DEFAULT_ABI, cache.param_types.size(),
...
FFICaller::CIFCache &
FFICaller::get_or_create_cif_variadic(const FFISignature &sig) {
...
Value FFICaller::call_function(void *func_ptr, const FFISignature &sig,
  ...
  // todo: this is a bit gross but we need to store the converted argument
  // values somewhere so we can pass pointers to them to ffi_call. we can't just
  // convert and pass on the fly because the storage needs to persist until
  // after ffi_call returns. we also need to handle string storage separately
  // since we need to keep the std::string objects alive while passing their
  // c_str() pointers to ffi.
...
Value FFICaller::call_function_variadic(void *func_ptr, const FFISignature &sig,
                                        const std::vector<Value> &args) {
  if (!func_ptr) {
    return Value::none();
  }
  ...
  for (size_t i = 0; i < args.size() && i < sig.param_types.size(); i++) {
    ...
    case FFIType::Struct: {
      ...
      fprintf(stderr,
              "WARNING: Struct parameters in variadic functions are not fully "
              "supported (parameter %zu)\n",
              i);
      break;
    }
...
#else
  printf("Unsupported platform for FFI! Falling back to linux semantics, but "
         "expect issues!");
  push_linux_search_paths(search_paths);
#endif
...
size_t get_struct_size(const std::string &type_name) {
  ...
  // prepare a dummy CIF that returns this struct to force libffi to compute its
  // layout
  ffi_cif cif;
  ffi_status status =
      ffi_prep_cif(&cif, FFI_DEFAULT_ABI, 0, ffi_struct, nullptr);
  if (status != FFI_OK) {
    fprintf(stderr,
            "ERROR: ffi_prep_cif failed in get_struct_size (status=%d)\n",
            status);
    return 0;
  }
...
Value read_struct_from_memory(void *ptr, const std::string &type_name) {
  ...
  for (const auto &field : struct_def->fields) {
    ...
    case FFIType::Pointer: {
      ...
      (*result_obj)[field.name] =
          Value::make_int(reinterpret_cast<int64_t>(ptr_val));
      offset += sizeof(void *);
      break;
    }
    default:
...
void write_struct_to_memory(void *ptr, const std::string &type_name,
  ...
  for (const auto &field : struct_def->fields) {
    ...
    if (field_it == obj_map->end()) {
      fprintf(stderr,
              "WARNING: Field '%s' not found in object for struct write\n",
              field.name.c_str());
      continue;
    }
...
void FFICallbackManager::callback_trampoline(ffi_cif *cif, void *ret,
  ...
  for (size_t i = 0; i < callback->signature.param_types.size(); i++) {
    ...
    case FFIType::UInt32:
      ...
      break;
    case FFIType::UInt64:
      nari_args.push_back(Value::make_int(
          static_cast<int64_t>(*static_cast<uint64_t *>(arg_ptr))));
      break;
    case FFIType::Float:
    ...
    case FFIType::Bool:
      nari_args.push_back(
          Value::make_bool(*static_cast<uint8_t *>(arg_ptr) != 0));
      break;
    case FFIType::Pointer: {
  ...
  // call nari func
  Value result = callback->runtime->call_function_value(callback->nari_function,
                                                        nari_args);
  ...
  case FFIType::Int8:
    *static_cast<int8_t *>(ret) =
        result.is_int() ? static_cast<int8_t>(result.get_int()) : 0;
    break;
  case FFIType::UInt8:
    *static_cast<uint8_t *>(ret) =
        result.is_int() ? static_cast<uint8_t>(result.get_int()) : 0;
    break;
  case FFIType::Int16:
    *static_cast<int16_t *>(ret) =
        result.is_int() ? static_cast<int16_t>(result.get_int()) : 0;
    break;
  case FFIType::UInt16:
    *static_cast<uint16_t *>(ret) =
        result.is_int() ? static_cast<uint16_t>(result.get_int()) : 0;
    break;
  case FFIType::Int32:
    *static_cast<int32_t *>(ret) =
        result.is_int() ? static_cast<int32_t>(result.get_int()) : 0;
    break;
  case FFIType::Int64:
  ...
  case FFIType::UInt32:
    *static_cast<uint32_t *>(ret) =
        result.is_int() ? static_cast<uint32_t>(result.get_int()) : 0;
    break;
  case FFIType::UInt64:
    *static_cast<uint64_t *>(ret) =
        result.is_int() ? static_cast<uint64_t>(result.get_int()) : 0;
    break;
  case FFIType::Float:
    if (result.is_float()) {
...
void *FFICallbackManager::create_callback(const FFISignature &sig,
                                          const Value &nari_func,
  ...
  callback->closure = static_cast<ffi_closure *>(
      ffi_closure_alloc(sizeof(ffi_closure), &callback->executable_code));
  if (!callback->closure) {
