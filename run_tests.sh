...
INTERP="build/${BUILD_TYPE}/interpreter"
NARIC="build/${BUILD_TYPE}/naric"

# Sanitizer runtime options. Caller-provided values win (so CI can override).
# This suite targets undefined-behaviour / use-after-free / out-of-bounds /
# shift classes (esp. the JIT codegen paths). Two ASan sub-features are off by
# design, not to hide bugs:
#  detect_leaks=0  - LSan has known false-positives in this tree.
#  detect_container_overflow=0 - the asmjit JIT manages vm->stack's end pointer
#  directly in emitted code (see asmjit_jit.cpp: stack._M_finish writes),
#  bypassing libc++'s push_back/pop_back, so the container size annotation
#  is intentionally stale. Without this, every JIT stack pop trips a
#  false container-overflow. Heap-overflow, UAF, stack-use-after-return and
#  all of UBSan remain fully active.
...
# SKIP_NETWORK_TESTS=1 drops tests that reach external hosts (flaky in CI).
# Currently just test_spawn_methods.nari (example.com/httpbin.org/etc).
...
# Per-test timeout. Most tests finish in well under a second, but a few
# (e.g. tests/expect_pass/test_spawn_methods.nari) hit real network endpoints
# and can stretch to several seconds under CPU load or slow DNS. The 60s cap
# stays safely above any legitimate run while still catching genuine hangs.
...
# (malformed bytecode files, CLI behavior, etc).
# EXTRA_SUITES=("tests/naric_robustness.sh" "tests/npkg_workspace_smoke.sh" "tests/npkg_publish_polish.sh" "tests/npkg_search_smoke.sh" "tests/npkg_owner_smoke.sh")
