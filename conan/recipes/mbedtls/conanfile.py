...
class MBedTLSConan(ConanFile):
    ...
    def build(self):
        # mbedtls 3.5.0's aesce.c (AES ARMv8 crypto-extension) doesn't compile
        # under MSVC/ARM64 -- it guards on __ARM_NEON (a GCC/Clang macro MSVC
        # doesn't define) and hard-errors. Fixed upstream in 3.6, but libcurl
        # pins 3.5.0. Disable MBEDTLS_AESCE_C on MSVC/ARM64 only (software AES
        # fallback); a no-op everywhere else, so it's applied unconditionally.
        replace_in_file(
            self,
            os.path.join(self.source_folder, "include", "mbedtls", "mbedtls_config.h"),
